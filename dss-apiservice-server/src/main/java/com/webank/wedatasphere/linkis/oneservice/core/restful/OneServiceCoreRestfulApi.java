...
import com.webank.wedatasphere.linkis.oneservice.core.service.OneServiceService;
import com.webank.wedatasphere.linkis.oneservice.core.vo.ApiVersionVo;
import com.webank.wedatasphere.linkis.oneservice.core.vo.OneServiceQuery;
...
//            }
    ...
    public Response query() {
        return ApiUtils.doAndResponse(() -> {
            ...
            List<String> tags= oneServiceService.queryAllTags();
            return Message.ok().data("tags", tags);
        }, "/oneservice/tags", "Fail to query page of service tag[查询服务tag失败]");
    }
    ...
    public Response queryById(@QueryParam("id") Long id) {
        return ApiUtils.doAndResponse(() -> {
            ...
            OneServiceVo oneServiceVo = oneServiceService.queryById(id);
            return Message.ok().data("result", oneServiceVo);
        }, "/oneservice/search", "Fail to query page of service api[查询服务api失败]");
    }
    ...
    @GET
    @Path("/apiParamQuery")
    public Response apiParamQuery(@QueryParam("id") Long id,
        ...
        return ApiUtils.doAndResponse(() -> {
            ...
            List<TestParamVo> testParamVoList = oneServiceQueryService.query(id, version);
    ...
    public Response apiVersionQuery(@QueryParam("id") Long id) {
        return ApiUtils.doAndResponse(() -> {
            if (id==null) {
                return Message.error("'one service api scriptPath' is missing[缺少api scriptPath]");
            }
            List<ApiVersionVo> apiVersionVoList = oneServiceQueryService.queryApiVersion(id);
