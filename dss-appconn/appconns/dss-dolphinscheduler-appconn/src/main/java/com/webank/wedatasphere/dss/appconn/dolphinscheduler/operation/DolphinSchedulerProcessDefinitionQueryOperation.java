...
public class DolphinSchedulerProcessDefinitionQueryOperation implements RefQueryOperation {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(DolphinSchedulerProcessDefinitionQueryOperation.class);
    ...
    public ResponseRef query(RequestRef ref) throws ExternalOperationFailedException {
        ...
        String releaseState =
                queryProcessDefinitionReleaseStateById(
                        dolphinProjectName,
                        (Long) requestRef.getParameter("processId"),
                        (String) requestRef.getParameter("username"));
        return new DolphinSchedulerProjectResponseRef(releaseState);
    }

    private String queryProcessDefinitionReleaseStateById(
            String projectName, Long processId, String userName)
            throws ExternalOperationFailedException {
        String queryUrl =
                StringUtils.replace(
                        this.queryProcessDefinitionByIdUrl, "${projectName}", projectName);
        ...
        try {
            ...
            DolphinSchedulerHttpGet httpGet =
                    new DolphinSchedulerHttpGet(uriBuilder.build(), userName);
            ...
            SchedulisExceptionUtils.dealErrorException(
                    90021, "获取工作流调度状态失败", e, ExternalOperationFailedException.class);
        } finally {
