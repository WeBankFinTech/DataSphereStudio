...
public class DolphinSchedulerPostRequestOperation
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(DolphinSchedulerPostRequestOperation.class);
    ...
    @Override
    public CloseableHttpResponse requestWithSSO(
            SSOUrlBuilderOperation urlBuilder, DolphinSchedulerHttpPost req)
            throws AppStandardErrorException {
        try {
            ArrayList<Header> headers = new ArrayList<>();
            Header header =
                    new BasicHeader(
                            "token", dolphinSchedulerSecurityService.getUserToken(req.getUser()));
            headers.add(header);
            ...
            logger.info(
                    "dolphin请求url"
                            + req.getURI()
                            + "token"
                            + dolphinSchedulerSecurityService.getUserToken(req.getUser()));
