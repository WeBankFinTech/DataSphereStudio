...
public class EventChecker implements Runnable {
    public static final String WAIT_TIME = "max.receive.hours";
    public static final String WAIT_FOR_TIME = "wait.for.time";
    public static final String QUERY_FREQUENCY = "query.frequency";
    public static final String MSGTYPE = "msg.type";
    public static final String SENDER = "msg.sender";
    public static final String RECEIVER = "msg.receiver";
    public static final String TOPIC = "msg.topic";
    public static final String MSGNAME = "msg.name";
    public static final String MSG = "msg.body";
    public static final String EXEC_ID = "azkaban.flow.execid";
    public static final String SAVE_KEY = "msg.savekey";
    public static final String USER_TIME = "msg.init.querytime";
    public static final String TODAY = "only.receive.today";
    public static final String AFTERSEND = "msg.after.send";
        ...
        try {
            ...
            if (checkParamMap(p, TOPIC)) {
                ...
                if (!topic.matches("[^_]*_[^_]*_[^_]*")) {
                    throw new RuntimeException(
                            "Error format of topic parameter. Accept: XX_XX_XX.");
                }
            }
            ...
            if (p.getProperty(MSGTYPE).equals("SEND")) {
                if (checkParamMap(p, SENDER)) {
                    ...
                    if (!sender.matches("[^@]*@[^@]*@[^@]*")) {
                        throw new RuntimeException(
                                "Error format of  sender parameter. Accept: XX@XX@XX.");
                    }
                }
                if (p.containsKey(MSG)
                        && StringUtils.isNotEmpty(p.getProperty(MSG))
                        && p.getProperty(MSG).length() > 250) {
                ...
                throw new RuntimeException(
                        "Please input correct parameter of msg.type, Select RECEIVE Or SEND.");
            }
        } catch (Exception ex) {
        ...
        if (p.getProperty(MSGTYPE).equals("RECEIVE")) {
            if (checkParamMap(p, RECEIVER)) {
                ...
                if (!receiver.matches("[^@]*@[^@]*@[^@]*")) {
                    backAction.setState(RefExecutionState.Failed);
                    throw new RuntimeException(
                            "Error format of receiver parameter. Accept: XX@XX@XX.");
                }
            }
    ...
    private boolean checkParamMap(Properties p, String key) {
        ...
        if (!checkNoStandardStr(p.getProperty(key))) {
            throw new RuntimeException(
                    "parameter " + key + " Accept letter and number and _@- only.");
        }
    ...
    private void checkTimeParam(Properties p, String key) {
        if (p.containsKey(key)) {
            ...
            if (!waitForTime.matches("^(0?[0-9]|1[0-9]|2[0-3]):(0?[0-9]|[1-5][0-9])$")) {
                throw new RuntimeException(
                        "Parameter " + key + " Time format error ! For example: HH:mm");
            }
        }
    }
    ...
    private String checkTimeParamMap(Properties p, String key) {
        if (p.containsKey(key)) {
            ...
            Pattern ptime =
                    Pattern.compile(
                            "^([1][7-9][0-9][0-9]|[2][0][0-9][0-9])(\\-)([0][1-9]|[1][0-2])(\\-)([0-2][1-9]|[3][0-1])(\\s)([0-1][0-9]|[2][0-3])(:)([0-5][0-9])(:)([0-5][0-9])$");
            Matcher m = ptime.matcher(userTime);
            if (!m.matches()) {
                throw new RuntimeException(
                        "Parameter "
                                + key
                                + " Time format error ! For example: yyyy-MM-dd HH:mm:ss");
            }
            return userTime;
        } else {
            return null;
