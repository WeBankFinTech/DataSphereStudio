...
public class OrchestratorFrameworkDeleteOperation
        implements RefDeletionOperation<OrchestratorDeleteRequestRef> {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(OrchestratorFrameworkDeleteOperation.class);

    @Override
    public void setDevelopmentService(DevelopmentService service) {}

    @Override
    public void deleteRef(OrchestratorDeleteRequestRef requestRef)
            throws ExternalOperationFailedException {
        LOGGER.info("Begin to ask to delete orchestrator, requestRef is {}", requestRef);
        RequestDeleteOrchestrator deleteRequest =
                new RequestDeleteOrchestrator(
        ...
        try {
            ...
            if (deleteResponse == null
                    || !deleteResponse.getJobStatus().equals(JobStatus.Success)) {
                LOGGER.error("delete response is null or delete response status is not success");
                DSSExceptionUtils.dealErrorException(
                        60075, "failed to delete ref", ExternalOperationFailedException.class);
            }
        } catch (Exception e) {
            DSSExceptionUtils.dealErrorException(
                    60015,
                    "delete orchestrator ref failed",
