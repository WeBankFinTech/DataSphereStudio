...
public class OrchestratorFrameworkImportOperation
        implements RefImportOperation<OrchestratorImportRequestRef> {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(OrchestratorFrameworkImportOperation.class);
    private DevelopmentService service;

    @Override
    public CommonResponseRef importRef(OrchestratorImportRequestRef requestRef)
            throws ExternalOperationFailedException {
        if (null == requestRef) {
            return null;
        }
        LOGGER.info(
                "Begin to ask to import orchestrator, requestRef is {}",
                DSSCommonUtils.COMMON_GSON.toJson(requestRef));
        RequestImportOrchestrator importRequest =
                new RequestImportOrchestrator(
        ...
        try {
            ...
            DSSExceptionUtils.dealErrorException(
                    60015,
                    "import orchestrator ref failed",
                    t,
                    ExternalOperationFailedException.class);
        }
        LOGGER.info(
                "End to ask to import orchestrator, responseRef is {}",
                DSSCommonUtils.COMMON_GSON.toJson(importResponse));
        ...
        if (null == importResponse) {
            ...
            DSSExceptionUtils.dealErrorException(
                    60015, "import ref response is null", ExternalOperationFailedException.class);
        }
