...
public class ProjectInfoWorkflowToRelConverter implements WorkflowToRelConverter {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(ProjectInfoWorkflowToRelConverter.class);

    @Override
    public ConvertedRel convertToRel(PreConversionRel rel) {
        List<String> repeatNode =
                AzkabanUtilsScala.getRepeatNodeName(getAllNodeName(rel.getWorkflows()));
    ...
    private void assignStorePath(AzkabanConvertedRel rel) {
        ...
        String storePath =
                AzkabanConf.DEFAULT_STORE_PATH.getValue()
                        + File.separator
                        + userName
                        + File.separator
                        + dateStr
        ...
        try {
            ...
            String projectZip =
                    projectDir.getParent()
                            + File.separator
                            + rel.getDSSToRelConversionRequestRef().getDSSProject().getName()
                            + ".zip";
    ...
    private List<String> getAllNodeName(List<Workflow> workflows) {
        ...
        workflows.forEach(
                flow -> flow.getWorkflowNodes().forEach(node -> nodeNames.add(node.getName())));
        return nodeNames;
    }
