...
public class VisualisRefExecutionOperation implements RefExecutionOperation {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(VisualisRefExecutionOperation.class);
    ...
    public ResponseRef execute(ExecutionRequestRef ref) throws ExternalOperationFailedException {
        ...
        String nodeType =
                asyncExecutionRequestRef
                        .getExecutionRequestRefContext()
                        .getRuntimeMap()
                        .get("nodeType")
                        .toString();
        if ("visualis.widget".equalsIgnoreCase(nodeType)) {
            ...
            return executePreview(
                    ...
                    URLUtils.getUrl(
                            getBaseUrl(),
                            URLUtils.DISPLAY_PREVIEW_URL_FORMAT,
                            getId(asyncExecutionRequestRef)),
                    URLUtils.getUrl(
                            getBaseUrl(),
                            URLUtils.DISPLAY_METADATA_URL_FORMAT,
                            getId(asyncExecutionRequestRef)));
            ...
            return executePreview(
                    ...
                    URLUtils.getUrl(
                            getBaseUrl(),
                            URLUtils.DASHBOARD_PREVIEW_URL_FORMAT,
                            getId(asyncExecutionRequestRef)),
                    URLUtils.getUrl(
                            getBaseUrl(),
                            URLUtils.DASHBOARD_METADATA_URL_FORMAT,
                            getId(asyncExecutionRequestRef)));
        } else {
            throw new ExternalOperationFailedException(
                    90177, "Unknown task type " + nodeType, null);
        }
    }

    private ResponseRef executeWidget(AsyncExecutionRequestRef ref)
        ...
        SSOUrlBuilderOperation ssoUrlBuilderOperation =
                ref.getWorkspace().getSSOUrlBuilderOperation().copy();
        ...
        try {
            ...
            HttpResult httpResult =
                    this.ssoRequestOperation.requestWithSSO(
                            ssoUrlBuilderOperation, visualisDownloadAction);
            WidgetResultData responseData =
                    BDPJettyServerHelper.gson()
                            .fromJson(
                                    IOUtils.toString(visualisDownloadAction.getInputStream()),
                                    WidgetResultData.class);
            if (responseData.getData().getColumns().isEmpty()) {
                ref.getExecutionRequestRefContext().appendLog("Cannot execute an empty Widget!");
                throw new ExternalOperationFailedException(
                        90176, "Cannot execute an empty Widget!", null);
            }
            for (WidgetResultData.Column columnData : responseData.getData().getColumns()) {
                columns.add(
                        new Column(
                                columnData.getName(),
                                DataType.toDataType(columnData.getType().toLowerCase()),
                                ""));
            }
            ResultSetWriter resultSetWriter =
                    ref.getExecutionRequestRefContext().createTableResultSetWriter();
    ...
    private ResponseRef executePreview(
            AsyncExecutionRequestRef ref, String previewUrl, String metaUrl)
            throws ExternalOperationFailedException {
        ...
        try {
            ...
            ssoUrlBuilderOperation.setWorkspace(ref.getWorkspace().getWorkspaceName());
            logger.info(
                    "got getSSOUrlBuilderOperation:"
                            + SSOIntegrationConf.gson().toJson(ssoUrlBuilderOperation));
            logger.info(
                    "got getSSOUrlBuilderOperation built url:"
                            + ssoUrlBuilderOperation.getBuiltUrl());
            previewDownloadAction.setURL(ssoUrlBuilderOperation.getBuiltUrl());
            HttpResult previewResult =
                    this.ssoRequestOperation.requestWithSSO(
                            ssoUrlBuilderOperation, previewDownloadAction);
            ...
            SSOUrlBuilderOperation ssoUrlBuilderOperationMeta =
                    ref.getWorkspace().getSSOUrlBuilderOperation().copy();
            ...
            HttpResult metaResult =
                    this.ssoRequestOperation.requestWithSSO(
                            ssoUrlBuilderOperationMeta, metadataDownloadAction);
            String metadata =
                    StringUtils.chomp(
                            IOUtils.toString(
                                    metadataDownloadAction.getInputStream(),
                                    ServerConfiguration.BDP_SERVER_ENCODING().getValue()));
            ResultSetWriter resultSetWriter =
                    ref.getExecutionRequestRefContext().createPictureResultSetWriter();
            ...
            ref.getExecutionRequestRefContext()
                    .appendLog("Failed to debug Display url " + previewUrl);
    ...
    private String getUser(AsyncExecutionRequestRef requestRef) {
        return requestRef
                .getExecutionRequestRefContext()
                .getRuntimeMap()
                .get("wds.dss.workflow.submit.user")
                .toString();
    }
        ...
        try {
            String executionContent =
                    BDPJettyServerHelper.jacksonJson()
                            .writeValueAsString(requestRef.getJobContent());
            String nodeType =
                    requestRef
                            .getExecutionRequestRefContext()
                            .getRuntimeMap()
            ...
            if ("visualis.display".equalsIgnoreCase(nodeType)) {
                VisualisCommonResponseRef displayCreateResponseRef =
                        new VisualisCommonResponseRef(executionContent);
