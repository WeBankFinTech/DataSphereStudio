...
public class VisualisRefCreationOperation implements RefCreationOperation<CreateRequestRef> {
    private static final Logger logger =
            LoggerFactory.getLogger(VisualisRefCreationOperation.class);
    ...
    public ResponseRef createRef(CreateRequestRef requestRef)
        ...
        if ("linkis.appconn.visualis.widget"
                .equalsIgnoreCase(visualisCreateRequestRef.getNodeType())) {
            ...
            throw new ExternalOperationFailedException(
                    90177, "Unknown task type " + visualisCreateRequestRef.getNodeType(), null);
        }
    }

    private ResponseRef sendWidgetRequest(NodeRequestRef requestRef)
            throws ExternalOperationFailedException {
        ...
        visualisPostAction.addRequestPayload(
                CSCommonUtils.CONTEXT_ID_STR,
                requestRef.getJobContent().get(CSCommonUtils.CONTEXT_ID_STR));
        if (requestRef.getJobContent().get("bindViewKey") != null) {
            ...
            if (StringUtils.isNotBlank(viewNodeName) && !"empty".equals(viewNodeName)) {
                viewNodeName =
                        getNodeNameByKey(
                                viewNodeName, (String) requestRef.getJobContent().get("json"));
        ...
        // cs
        VisualisRefUpdateOperation visualisRefUpdateOperation =
                new VisualisRefUpdateOperation(developmentService);
        NodeUpdateCSRequestRefImpl visualisUpdateCSRequestRef = new NodeUpdateCSRequestRefImpl();
        visualisUpdateCSRequestRef.setContextID(
                (String) requestRef.getJobContent().get(CSCommonUtils.CONTEXT_ID_STR));
    ...
    public static String getNodeNameByKey(String key, String json) {
        ...
        List<DSSNode> dwsNodes =
                DSSCommonUtils.COMMON_GSON.fromJson(
                        nodeJsonArray, new TypeToken<List<DSSNodeDefault>>() {}.getType());
        return dwsNodes.stream()
                .filter(n -> key.equals(n.getId()))
                .map(DSSNode::getName)
                .findFirst()
                .orElse("");
    }

    private ResponseRef sendDisplayRequest(NodeRequestRef requestRef)
            throws ExternalOperationFailedException {
    ...
    private ResponseRef sendDashboardRequest(NodeRequestRef requestRef)
            throws ExternalOperationFailedException {
    ...
    private void createDisplaySlide(
            VisualisCommonResponseRef displayCreateResponseRef, NodeRequestRef requestRef)
            throws ExternalOperationFailedException {
        String url =
                getBaseUrl()
                        + URLUtils.displayUrl
        ...
        visualisPostAction.addRequestPayload(
                "displayId", Long.parseLong(displayCreateResponseRef.getDisplayId()));
        ...
        try {
            ...
            resMap =
                    BDPJettyServerHelper.jacksonJson()
                            .readValue(httpResult.getResponseBody(), Map.class);
        } catch (Exception e) {
    ...
    private void createDashboard(
            VisualisCommonResponseRef dashboardCreateResponseRef, NodeRequestRef requestRef)
            throws ExternalOperationFailedException {
        String url =
                getBaseUrl()
        ...
        visualisPostAction.addRequestPayload(
                "dashboardPortalId", Long.parseLong(dashboardCreateResponseRef.getDashboardId()));
        visualisPostAction.addRequestPayload("index", 0);
        visualisPostAction.addRequestPayload("name", requestRef.getName());
