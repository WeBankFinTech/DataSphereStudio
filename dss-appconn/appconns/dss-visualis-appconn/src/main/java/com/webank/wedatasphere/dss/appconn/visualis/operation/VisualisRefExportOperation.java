...
public class VisualisRefExportOperation implements RefExportOperation<ExportRequestRef> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(VisualisRefExportOperation.class);
        ...
        try {
            externalContent =
                    BDPJettyServerHelper.jacksonJson()
                            .writeValueAsString(requestRef.getParameter("jobContent"));
            if ("linkis.appconn.visualis.widget".equalsIgnoreCase(nodeType)) {
                ...
                visualisPostAction.addRequestPayload(
                        "widgetIds",
                        ((Double) Double.parseDouble(widgetCreateResponseRef.getWidgetId()))
                                .longValue());
                ...
                visualisPostAction.addRequestPayload(
                        "displayIds",
                        ((Double) Double.parseDouble(displayCreateResponseRef.getDisplayId()))
                                .longValue());
                ...
                visualisPostAction.addRequestPayload(
                        "dashboardPortalIds",
                        ((Double) Double.parseDouble(dashboardCreateResponseRef.getDashboardId()))
                                .longValue());
            } else {
                throw new ExternalOperationFailedException(
                        90177, "Unknown task type " + requestRef.getType(), null);
            }
        } catch (Exception e) {
