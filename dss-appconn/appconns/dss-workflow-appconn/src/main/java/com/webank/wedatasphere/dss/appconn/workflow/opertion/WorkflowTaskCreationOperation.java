...
public class WorkflowTaskCreationOperation
        implements RefCreationOperation<DefaultOrchestratorCreateRequestRef> {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(WorkflowTaskCreationOperation.class);
    ...
    @Override
    public CommonResponseRef createRef(DefaultOrchestratorCreateRequestRef workflowCreateRequestRef)
        ...
        String contextIDStr =
                workflowCreateRequestRef.getContextID() != null
        ...
        List<String> linkedAppConnNames =
                workflowCreateRequestRef.getDSSOrchestratorInfo().getLinkedAppConnNames() != null
                        ? workflowCreateRequestRef.getDSSOrchestratorInfo().getLinkedAppConnNames()
                        : new ArrayList<>();
        String uses =
                workflowCreateRequestRef.getDSSOrchestratorInfo().getUses() != null
                        ? workflowCreateRequestRef.getDSSOrchestratorInfo().getUses()
                        : "uses";
        RequestCreateWorkflow requestCreateWorkflow =
                new RequestCreateWorkflow(
                        userName,
                        workflowName,
                        contextIDStr,
                        description,
        ...
        if (null != sender) {
            ResponseCreateWorkflow responseCreateWorkflow =
                    (ResponseCreateWorkflow) sender.ask(requestCreateWorkflow);
            ...
            DSSExceptionUtils.dealErrorException(
                    61123,
                    "dss workflow server dev sender is null",
