...
public abstract class AbstractAppConn implements AppConn {
    ...
    protected final List<String> appStandardMethodHeader =
            Arrays.asList("create", "getOrCreate", "get");
    ...
    /**
     * Specification: each appconn needs to define a method starting with create and returning as
     * standard type to initialize the specifications owned by appconn.
     */
    @Override
    public final void init() throws AppConnErrorException {
                ...
                Arrays.stream(getClass().getDeclaredMethods())
                        .map(
                                method -> {
                                    ...
                                    if (appStandardMethodHeader.stream()
                                                    .anyMatch(methodName::startsWith)
                                            && AppStandard.class.isAssignableFrom(
                                        ...
                                        try {
                                            ...
                                            logger.warn(
                                                    methodName
                                                            + " execute failed, ignore to set it into appStandardList of "
        ...
        if (this instanceof OnlySSOAppConn) {
            SSORequestService ssoRequestService =
                    ((OnlySSOAppConn) this).getOrCreateSSOStandard().getSSORequestService();
            for (AppStandard appStandard : appStandards) {
                if (appStandard instanceof AppIntegrationStandard) {
                    ((AppIntegrationStandard<SSORequestService>) appStandard)
                            .setSSORequestService(ssoRequestService);
                }
                try {
                    appStandard.init();
                } catch (AppStandardErrorException e) {
                    throw new AppConnErrorException(
                            e.getErrCode(),
                            "Init " + appStandard.getStandardName() + " failed!",
                            e);
                }
            }
        }
