...
public interface ApiConfigMapper extends BaseMapper<ApiConfig> {
    ...
    @Insert(
            "insert into dss_dataapi_group(workspace_id, name, note, create_by) "
    ...
    @Select(
            "select id,api_name as name,req_fields,api_path as path from dss_dataapi_config where group_id = #{groupId}")
    List<ApiListInfo> getApiListByGroup(@Param("groupId") int groupId);

    List<ApiInfo> getApiInfoList(
            @Param("workspaceId") Long workspaceId, @Param("apiName") String apiName);

    List<ApiInfo> getOnlineApiInfoList(
            @Param("workspaceId") Long workspaceId, @Param("apiName") String apiName);
    ...
    @Select(
            "SELECT COUNT(1) FROM dss_dataapi_config WHERE `is_delete` = 0 AND `status` = 1 AND `workspace_id` = #{workspaceId}")
    Long getOnlineApiCnt(Long workspaceId);

    @Select(
            "SELECT COUNT(1) FROM dss_dataapi_config WHERE `is_delete` = 0 AND `status` = 0 AND `workspace_id` = #{workspaceId}")
