...
public class DSSDbApiDataSourceRestful {
    ...
    @Resource private ApiDataSourceService dssDbApiDataSourceService;
    ...
    public Message connect(
            @RequestParam("workspaceId") Integer workspaceId, @RequestParam("type") String type) {

        List<DataSource> allConnections =
                dssDbApiDataSourceService.getAllConnections(workspaceId, type);
        return Message.ok().data("availableConns", allConnections);
    }

    @RequestMapping(path = "tables", method = RequestMethod.GET)
    public Message getAllTables(@RequestParam("datasourceId") Integer datasourceId)
            throws SQLException {
    ...
    public Message getAllCols(
            @RequestParam("datasourceId") Integer datasourceId,
            @RequestParam("tableName") String tableName)
            throws SQLException {
        DataSource dataSource = dssDbApiDataSourceService.selectById(datasourceId);
        DruidPooledConnection connection = PoolManager.getPooledConnection(dataSource);
        List<HashMap> columns =
                JdbcUtil.getRDBMSColumnProperties(connection, dataSource.getType(), tableName);
        return Message.ok().data("allCols", columns);
    }

    @RequestMapping(path = "add", method = RequestMethod.POST)
    public Message addDatasource(
            @RequestBody DataSource dataSource, @Context HttpServletRequest req) {
