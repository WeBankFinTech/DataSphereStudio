...
public class ApiConfigServiceImpl extends ServiceImpl<ApiConfigMapper, ApiConfig>
    ...
    @Autowired ApiConfigMapper dssApiConfigMapper;
    @Autowired ApiAuthMapper apiAuthMapper;
    @Autowired ApiCallMapper apiCallMapper;
    @Autowired ApiConfigMapper apiConfigMapper;
    @Autowired ApiDataSourceService apiDataSourceService;
    ...
    public void saveApi(ApiConfig apiConfig) throws JSONException, DataApiException {
        ...
        List<ApiConfig> apiList =
                this.list(new QueryWrapper<ApiConfig>().eq("api_path", apiConfig.getApiPath()));
        ...
        if ("GUIDE".equals(apiType)) {
            ...
            String whereCause =
                    StringUtils.isBlank(reqFields) ? "" : CommUtil.getWhereCause(reqFields);
            String orderCause =
                    StringUtils.isBlank(orderFields) ? "" : CommUtil.getOrderCause(orderFields);
            String sql =
                    String.format(
                            "%s%s%s%s%s%s",
                            "select ", resFields, " from ", table, whereCause, orderCause);
     ...
     */
    @Override
    public ApiExecuteInfo apiTest(
            String path, HttpServletRequest request, Map<String, Object> map, boolean isTest)
            throws JSONException, SQLException, DataApiException {
     ...
     */
    @Override
    public ApiExecuteInfo apiExecute(
            String path, HttpServletRequest request, Map<String, Object> map)
            throws DataApiException, JSONException, SQLException {
        ...
        ApiConfig apiConfig =
                this.getOne(new QueryWrapper<ApiConfig>().eq("api_path", path).eq("is_delete", 0));
        if (apiConfig != null) {
    ...
    private Map<String, Object> getSqlParam(
            HttpServletRequest request, ApiConfig config, Map<String, Object> paraMap)
            throws JSONException {
        ...
        if (StringUtils.isNotBlank(reqFields)) {
            ...
            for (int i = 0; i < requestParams.length(); i++) {
                ...
                if (type.startsWith("Array")) {
                    ...
                    String value =
                            paraMap == null || paraMap.get(name) == null
                                    ? null
                                    : String.valueOf(paraMap.get(name));
    ...
    public ApiExecuteInfo executeSql(
            int isSelect,
            ...
            int pageSize,
            int pageNum)
            throws DataApiException, SQLException {
