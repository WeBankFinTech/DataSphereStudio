...
public class MetaInfoMapperImpl implements MetaInfoMapper {
    ...
    public Long getTableStorage() throws SQLException {
        ...
        try {
            ...
            String sql2 =
                    "select SUM(PARAM_VALUE) from PARTITION_PARAMS WHERE PARAM_KEY='totalSize'";
    ...
    public List<HiveStorageInfo> getTop10Table() throws SQLException {
        ...
        try {
            String sql =
                    "SELECT DBS.NAME ,TBLS.TBL_NAME,CAST(TABLE_PARAMS.PARAM_VALUE AS UNSIGNED) AS totalSize from DBS, TBLS,TABLE_PARAMS where TBLS.TBL_ID=TABLE_PARAMS.TBL_ID AND TBLS.DB_ID=DBS.DB_ID AND TABLE_PARAMS.PARAM_KEY='totalSize'  order by totalSize DESC limit 10";
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
            ...
            /**
             * 特别注意LONG类型相减超出INT范围 System.out.println((int) (4401131805L -1796673800L))
             * System.out.println(Long.parseLong("4401131805")-Long.parseLong("1796673800"))
             */
            Collections.sort(
                    hiveStorageInfos,
                    new Comparator<HiveStorageInfo>() {
                        @Override
                        public int compare(HiveStorageInfo o1, HiveStorageInfo o2) {
                            // return (int)
                            // (Long.valueOf(o2.getStorage())-Long.valueOf(o1.getStorage()))
    ...
    @Override
    public int getTableInfo(String dbName, String tableName, Boolean isPartTable)
            throws SQLException {
        DataSource dataSource = DataSourceUtil.getDataSource();
        Connection con = dataSource.getConnection();
        PreparedStatement ps = null;
        ...
        try {
            ...
            if (isPartTable == false) {
                sql =
                        "select TABLE_PARAMS.PARAM_VALUE as totalSize from DBS, TBLS,TABLE_PARAMS where TBLS.TBL_ID=TABLE_PARAMS.TBL_ID AND TBLS.DB_ID=DBS.DB_ID AND TABLE_PARAMS.PARAM_KEY='totalSize' AND  DBS.NAME="
                                + "'"
                                + dbName
                                + "' AND  TBLS.TBL_NAME="
                                + "'"
                ...
                sql =
                        "select SUM(PARTITION_PARAMS.PARAM_VALUE) as totalSize  from DBS,TBLS,PARTITIONS ,PARTITION_PARAMS where DBS.DB_ID=TBLS.DB_ID AND TBLS.TBL_ID=PARTITIONS.TBL_ID AND  PARTITIONS.PART_ID =PARTITION_PARAMS.PART_ID  AND PARTITION_PARAMS.PARAM_KEY='totalSize' AND DBS.NAME="
                                + "'"
                                + dbName
                                + "' AND TBLS.TBL_NAME="
                                + "'"
    ...
    public List<HivePartInfo> getPartInfo(String dbName, String tableName) throws SQLException {
        ...
        try {
            String sql =
                    "select b.PART_NAME,b.CREATE_TIME,MAX(CASE c.PARAM_KEY WHEN 'transient_lastDdlTime' THEN c.PARAM_VALUE ELSE null END) transient_lastDdlTime ,MAX(CASE c.PARAM_KEY WHEN 'numRows' THEN c.PARAM_VALUE ELSE null END) numRows,MAX(CASE c.PARAM_KEY WHEN 'totalSize' THEN c.PARAM_VALUE ELSE null END) totalSize   from TBLS a,PARTITIONS b,PARTITION_PARAMS c,DBS d where  a.TBL_NAME="
                            + "'"
                            + tableName
                            + "'"
                            + "AND d.NAME="
                            ...
                            + dbName
                            + "'"
                            + "AND a.TBL_ID=b.TBL_ID AND a.DB_ID=d.DB_ID AND b.PART_ID=c.PART_ID  GROUP BY c.PART_ID";
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
