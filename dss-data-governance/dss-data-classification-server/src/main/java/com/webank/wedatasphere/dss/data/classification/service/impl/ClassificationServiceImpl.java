...
/**
 * @author suyc @Classname ClassificationServiceImpl @Description TODO @Date 2021/9/24
 *     10:27 @Created by suyc
 */
@Service
@AllArgsConstructor
    ...
    private void initializeClassification(String name, String description) {
        ...
        try {
            ...
            if (atlasClassificationDef != null) {
                logger.info(
                        String.format(
                                "@@@@@@@@@@@@@@ === this classification [name=%s] exists,  ignore it",
                                name));
            }
            ...
            if (ClientResponse.Status.NOT_FOUND == ex.getStatus()) {
                logger.info(
                        String.format(
                                "@@@@@@@@@@@@@@ === this classification [name=%s] don't exist,  create it...",
                                name));
                logger.info(
                        String.format(
                                "@@@@@@@@@@@@@@ ==> initializeClassification, name=%s", name));
