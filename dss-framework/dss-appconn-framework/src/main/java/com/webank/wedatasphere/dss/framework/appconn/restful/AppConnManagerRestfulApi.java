...
public class AppConnManagerRestfulApi {
    ...
    @Autowired private AppConnInfoService appConnInfoService;
    @Autowired private AppConnResourceUploadService appConnResourceUploadService;
        ...
        appConnInfoService
                ...
                .forEach(
                        DSSExceptionUtils.handling(
                                appConnInfo -> {
                                    LOGGER.info(
                                            "Try to load or update AppConn {}.",
                                            appConnInfo.getAppConnName());
                                    if (!"local".equals(System.getProperty("env"))) {
                                        appConnResourceUploadService.upload(
                                                appConnInfo.getAppConnName());
                                    }
                                }));
    ...
    public Message getAppInstancesByAppConnInfo(@PathVariable("appConnName") String appConnName) {
        List<? extends AppInstanceInfo> appInstanceInfos =
