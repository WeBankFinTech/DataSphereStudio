...
public class DssAuditController {
    ...
    public Message getScriptDownload(
            @RequestParam(required = false) String userName,
            @RequestParam(required = false) String startTime,
            @RequestParam(required = false) String endTime,
            @RequestParam Integer pn) {
        PageHelper.startPage(pn, 10, true);
        List<DssScriptDownloadAudit> userPage =
                dssScriptDownloadService.getDownloadAuditList(userName, startTime, endTime);
