...
/** @Auther: Han Tang @Date: 2022/1/11-01-11-14:20 */
@RestController
@RequestMapping(
        path = "/dss/framework/exchangis",
...
public class DssExchangisController {
    ...
    @Autowired DssExchangeService dssExchangeService;
    ...
    public Message getTaskTree(
        ...
        String userName = SecurityFilter.getLoginUsername(request);
        //        String userName = "admin";
        DssExchangeTaskRes dssExchangeTaskRes =
                dssExchangeService.queryExchangeTask(projectId, userName, pageNum, fuzzyName);
        Message message = Message.ok().data("response", dssExchangeTaskRes);
        return message;
    }

    @RequestMapping(path = "/shell", method = RequestMethod.GET)
    public Message getShellScript(
            @Context HttpServletRequest request,
