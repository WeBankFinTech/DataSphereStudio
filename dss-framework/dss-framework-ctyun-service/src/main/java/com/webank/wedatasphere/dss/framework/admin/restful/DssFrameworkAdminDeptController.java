...
public class DssFrameworkAdminDeptController {
    @Resource private DssAdminDeptService dssAdminDeptService;
    ...
    public Message add(@RequestBody DssAdminDept dssAdminDept) {
        ...
        if (UserConstants.NOT_UNIQUE.equals(
                dssAdminDeptService.checkDeptNameUnique(dssAdminDept))) {
            ...
            return Message.error()
                    .message("新增部门'" + dssAdminDept.getDeptName() + "'失败，该部门是末级部门,不能新增下级部门");
        }
    ...
    public Message treeselect(DssAdminDept dept) {
        ...
        return Message.ok()
                .data("deptTree", dssAdminDeptService.buildDeptTreeSelect(depts))
                .message("树形部门获取成功");
    }
