...
/** @Auther: Han Tang @Date: 2022/1/18-01-18-15:10 */
@Service
public class DssExchangeService {
    ...
    public List<DssExchangisProject> queryExchangeProject(String userName) throws Exception {
        ...
        List<DssExchangisProject> dssExchangisProjects =
                getExchangeProjectList(projectList, jsonArray);
        return dssExchangisProjects;
    }

    public DssExchangeTaskRes queryExchangeTask(
            int projectId, String userName, int pageNum, String fullName) throws Exception {
        String url =
                ProjectConf.EXCHANGE_URL.getValue()
                        + TASK_TREE_PATH
                        + "/"
                        + userName
                        + "?projectId="
        ...
        if (resJsonArray != null) {
            for (JsonElement jsonElement : resJsonArray) {
                ...
                dssExchangeTask.setJobName(
                        jsonObject.get("jobName") == null
                                ? null
                                : jsonObject.get("jobName").getAsString());
                dssExchangeTask.setJobCorn(
                        jsonObject.get("jobCorn") == null
                                ? null
                                : jsonObject.get("jobCorn").getAsString());
                dssExchangeTask.setJobDesc(
                        jsonObject.get("jobDesc") == null
                                ? null
                                : jsonObject.get("jobDesc").getAsString());
                dssExchangeTask.setCreateTime(
                        jsonObject.get("createTime") == null
                                ? null
                                : jsonObject.get("createTime").getAsString());
                dssExchangeTask.setJobStatus(
                        jsonObject.get("jobStatus") == null
                                ? null
                                : jsonObject.get("jobStatus").getAsString());
    ...
    public String getSellScript(int taskId, int projectId) {
        String shellScript =
                "str=`curl -X GET --data '{\"project_id\":"
                        + projectId
                        + ",\"task_id\":"
                        + taskId
                        + "}' "
                        ...
                        + "--header 'Cookie:"
                        + ProjectConf.EXCHANGE_ADMIN_COOKIE.getValue()
                        + "' "
                        + ProjectConf.EXCHANGE_URL.getValue()
                        + "/api/v1/jobinfo/runTask/"
                        + taskId
                        + "?userName=admin`;if [[ ${str} =~ 'job execution successed' ]];then exit 0;else exit 1;fi";
        return shellScript;
    }

    public List<DssExchangisProject> getExchangeProjectList(
            List<DssExchangisProject> projectList, JsonArray jsonArray) {
        if (jsonArray != null) {
            for (JsonElement jsonElement : jsonArray) {
                DssExchangisProject dssExchangisProject = new DssExchangisProject();
                dssExchangisProject.setProjectName(
                        jsonElement.getAsJsonObject().get("projectName") == null
                                ? null
                                : jsonElement.getAsJsonObject().get("projectName").getAsString());
                dssExchangisProject.setId(
