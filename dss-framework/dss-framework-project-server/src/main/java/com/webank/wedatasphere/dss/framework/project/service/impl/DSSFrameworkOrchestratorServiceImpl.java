...
public class DSSFrameworkOrchestratorServiceImpl implements DSSFrameworkOrchestratorService {
    ...
    @Autowired private DSSProjectService projectService;
    @Autowired private DSSWorkspaceService dssWorkspaceService;

    /**
     * 1.拿到的dss orchestrator的appconn 2.然后创建
     *
    ...
    public CommonOrchestratorVo createOrchestrator(
            String username,
            OrchestratorCreateRequest orchestratorCreateRequest,
            Workspace workspace)
            throws Exception {
        // 是否存在相同的编排名称
        orchestratorService.isExistSameNameBeforeCreate(
                orchestratorCreateRequest.getProjectId(),
                orchestratorCreateRequest.getWorkspaceId(),
                orchestratorCreateRequest.getOrchestratorName());
        // 判断工程是否存在,并且取出工程名称和空间名称
        ProjectInfoVo projectInfoVo =
                projectService.getProjectInfoById(orchestratorCreateRequest.getProjectId());
        ...
        try {
            OrchestratorKindEnum orchestratorKindEnum =
                    OrchestratorKindEnum.getType(
                            OrchestratorTypeEnum.getTypeByKey(
                                    orchestratorCreateRequest.getOrchestratorMode()));

            List<DSSLabel> dssLabels =
                    Arrays.asList(
                            new EnvDSSLabel(orchestratorCreateRequest.getLabels().getRoute()));
            ...
            orchestratorCreateRequestRef.setProjectName(
                    StringUtils.isBlank(orchestratorCreateRequest.getProjectName())
                            ? projectInfoVo.getProjectName()
                            : orchestratorCreateRequest.getProjectName());
            orchestratorCreateRequestRef.setWorkspaceName(
                    StringUtils.isBlank(orchestratorCreateRequest.getWorkspaceName())
                            ? projectInfoVo.getWorkspaceName()
                            : orchestratorCreateRequest.getWorkspaceName());
            ...
            dssOrchestratorInfo.setSecondaryType(
                    orchestratorCreateRequest.getOrchestratorWays().toString());
            ...
            OrchestratorCreateResponseRef responseRef =
                    (OrchestratorCreateResponseRef)
                            refCreationOperation.createRef(orchestratorCreateRequestRef);
            ...
            DSSExceptionUtils.dealErrorException(
                    60033, "failed to create orchestrator", e, DSSProjectErrorException.class);
        }
        return orchestratorVo;
    }

    @Override
    public CommonOrchestratorVo modifyOrchestrator(
            String username,
        ...
        // 判断工程是否存在,并且取出工程名称和空间名称
        ProjectInfoVo projectInfoVo =
                projectService.getProjectInfoById(orchestratorModifyRequest.getProjectId());
        if (projectInfoVo == null) {
            DSSExceptionUtils.dealErrorException(
                    ProjectServerResponse.PROJECT_NOT_EXIST.getCode(),
                    ProjectServerResponse.PROJECT_NOT_EXIST.getMsg(),
        ...
        // 是否存在相同的编排名称
        Long orchestratorId =
                orchestratorService.isExistSameNameBeforeUpdate(orchestratorModifyRequest);
        // 1.去orchestratorFramework创建编排模式
        // 2.将工程和orchestrator的关系存储到的数据库中
        ...
        try {
            OrchestratorKindEnum orchestratorKindEnum =
                    OrchestratorKindEnum.getType(
                            OrchestratorTypeEnum.getTypeByKey(
                                    orchestratorModifyRequest.getOrchestratorMode()));
            List<DSSLabel> dssLabels =
                    Arrays.asList(
                            new EnvDSSLabel(orchestratorModifyRequest.getLabels().getRoute()));

            String appconnName = "workflow";
            WorkflowOrchestratorUpdateRequestRef orchestratorUpdateRequestRef =
                    new WorkflowOrchestratorUpdateRequestRef();
            switch (orchestratorKindEnum) {
                case COMBINED:
                    appconnName = "combined";
                    break;
            ...
            dssOrchestratorInfo.setSecondaryType(
                    orchestratorModifyRequest.getOrchestratorWays().toString());
            ...
            // 根据label获取对应label的创建operation
            RefUpdateOperation<WorkflowOrchestratorUpdateRequestRef> updateOperation =
                    getRefCreationOperation(dssLabels).getRefUpdateOperation();
            // 调用orchestrator服务 更新编排模式、编排模式版本
            LOGGER.info("----orchestratorUpdateRequestRef---> {}", orchestratorUpdateRequestRef);
            CommonResponseRef responseRef =
                    (CommonResponseRef) updateOperation.updateRef(orchestratorUpdateRequestRef);
            LOGGER.info(
                    "operation ends to updateOrchestrator ref, UpdateResult is {},ErrorMsg is {}",
                    responseRef.getResult(),
            ...
            DSSExceptionUtils.dealErrorException(
                    60034, "failed to create orchestrator", e, DSSProjectErrorException.class);
        }
        return orchestratorVo;
    }
    ...
    public void deleteOrchestrator(
        ...
        if (deleteSchedulerWorkflow) {
            SchedulerAppConn schedulerAppConn =
                    (SchedulerAppConn)
                            AppConnManager.getAppConnManager()
                                    .getAppConn(
                                            DSSProjectConstant.DSS_SCHEDULER_APPCONN_NAME
                                                    .getValue());
            if (schedulerAppConn == null) {
                LOGGER.error(
                        "DolphinScheduler appconn is null, can not get scheduler workflow status");
                DSSExceptionUtils.dealErrorException(
                        61123, "scheduler appconn is null", DSSErrorException.class);
            }

            StructureIntegrationStandard schedulerStructureStandard =
                    schedulerAppConn.getOrCreateStructureStandard();
            ...
            ProjectDeletionOperation deletionOperation =
                    schedulerStructureStandard
                            .getProjectService(schedulerInstance)
                            .getProjectDeletionOperation();

            String workspaceName =
                    dssWorkspaceService.getWorkspaceName(
                            String.valueOf(dssOrchestrator.getWorkspaceId()));
        ...
        // TODO: 2021/6/4
        // 删除编排信息dss_orchestrator_info、版本信息dss_orchestrator_info_version、版本信息对应的工作流信息dss_workflow
    }

    @Override
    public CommonOrchestratorVo deleteOrchestrator(
            String username,
        ...
        // 判断工程是否存在,并且取出工程名称和空间名称
        ProjectInfoVo projectInfoVo =
                projectService.getProjectInfoById(orchestratorDeleteRequest.getProjectId());
        if (projectInfoVo == null) {
            DSSExceptionUtils.dealErrorException(
                    ProjectServerResponse.PROJECT_NOT_EXIST.getCode(),
                    ProjectServerResponse.PROJECT_NOT_EXIST.getMsg(),
        ...
        DSSOrchestrator orchestrator =
                orchestratorService.getOrchestratorById(orchestratorDeleteRequest.getId());
        // 1.去orchestratorFramework创建编排模式
        // 2.将工程和orchestrator的关系存储到的数据库中
        ...
        try {
            ...
            if (StringUtils.isBlank(uuid)) {
                DSSExceptionUtils.dealErrorException(
                        60035, "工作流不存在", DSSProjectErrorException.class);
            }
            List<DSSLabel> dssLabels =
                    Arrays.asList(
                            new EnvDSSLabel(orchestratorDeleteRequest.getLabels().getRoute()));
            WorkflowOrchestratoDeleteRequestRef orchestratorDeleteRequestRef =
                    new WorkflowOrchestratoDeleteRequestRef();
            String workspaceName = projectInfoVo.getWorkspaceName();
            ...
            DSSExceptionUtils.dealErrorException(
                    60035, "failed to delete orchestrator", e, DSSProjectErrorException.class);
        }
        return orchestratorVo;
    }
     ...
     * 获取更新的operation
     *
     * @param dssLabels 根据label获取对应label的创建operation todo 目前已经写死
     * @return
     * @throws Exception
     */
    public RefCRUDService getRefCreationOperation(List<DSSLabel> dssLabels) throws Exception {
        OrchestratorFrameworkAppConn orcAppConn =
                AppConnManager.getAppConnManager().getAppConn(OrchestratorFrameworkAppConn.class);

        if (orcAppConn == null) {
            LOGGER.error("orcAppConn is null, will not go on creating orchestrator");
            DSSExceptionUtils.dealErrorException(
                    60028, "orcAppConn is null", DSSProjectErrorException.class);
        }

        DevelopmentIntegrationStandard standard =
                ((OnlyDevelopmentAppConn) orcAppConn).getOrCreateDevelopmentStandard();
        if (standard == null) {
            LOGGER.error("standard is null, will not go on creating orchestrator");
            DSSExceptionUtils.dealErrorException(
                    60028, "standard is null", DSSProjectErrorException.class);
        }
