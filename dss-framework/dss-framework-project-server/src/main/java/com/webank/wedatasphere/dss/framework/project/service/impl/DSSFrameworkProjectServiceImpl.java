...
public class DSSFrameworkProjectServiceImpl implements DSSFrameworkProjectService {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(DSSFrameworkProjectServiceImpl.class);
    ...
    private static final boolean STRICT_PROJECT_CREATE_MODE =
            CommonVars.apply("wds.dss.project.strict.mode", false).getValue();
    ...
    public DSSProjectVo createProject(
        ...
        if (dbProject != null) {
            DSSExceptionUtils.dealErrorException(
                    60022,
                    String.format(
                            "project name already has the same name %s ",
                            projectCreateRequest.getName()),
                    DSSProjectErrorException.class);
        }
        // todo 创建appconn的相关的工程 还没有调试通过
        Map<AppInstance, Long> projectMap =
                createAppConnProject(projectCreateRequest, username, workspace);
        if (null == projectMap) {
            LOGGER.error("projectMap is null, it means some appConns create project failed");
            throw new DSSProjectErrorException(
                    71000, "projectMap is null, create project in appconn failed");
        }
        // 3.保存dss_project
    ...
    @Override
    public void modifyProject(ProjectModifyRequest projectModifyRequest, String username)
            throws Exception {
        ...
        if (dbProject == null) { // 工程不存在
            ...
            DSSExceptionUtils.dealErrorException(
                    60021,
                    String.format(
                            "%s project id is null, can not modify",
        ...
        if (!dbProject.getName().equalsIgnoreCase(projectModifyRequest.getName())) {
            DSSExceptionUtils.dealErrorException(
                    ProjectServerResponse.PROJECT_NOT_EDIT_NAME.getCode(),
                    ProjectServerResponse.PROJECT_NOT_EDIT_NAME.getMsg(),
                    DSSProjectErrorException.class);
        }
    ...
    public void deleteProject(
        ...
        if (projectDeleteRequest.isIfDelOtherSys()) {
            ...
            AppConnManager.getAppConnManager().listAppConns().stream()
                    .filter(appConn -> appConn instanceof OnlyStructureAppConn)
                    .forEach(
                            appConn -> {
                                OnlyStructureAppConn structureAppConn =
                                        (OnlyStructureAppConn) appConn;
                                appConn.getAppDesc()
                                        .getAppInstances()
                                        .forEach(
    ...
    private Map<AppInstance, Long> createAppConnProject(
            ProjectCreateRequest dssProjectCreateRequest, String username, Workspace workspace) {
        ...
        for (AppConn appConn : AppConnManager.getAppConnManager().listAppConns()) {
            if (appConn instanceof OnlyStructureAppConn) {
                ...
                StructureIntegrationStandard appStandard =
                        onlyStructureAppConn.getOrCreateStructureStandard();
                ...
                for (AppInstance appInstance : appConn.getAppDesc().getAppInstances()) {
                    ProjectService appStandardProjectService =
                            appStandard.getProjectService(appInstance);
                    ...
                    try {
                        LOGGER.info(
                                "Begin to create project {} in {}",
                                dssProjectCreateRequest.getName(),
                                appConn.getAppDesc().getAppName());
                        ProjectResponseRef responseRef =
                                projectCreationOperation.createProject(requestRef);
                        LOGGER.info(
                                "End to create project {} in {}, response projectId is {} ",
                                dssProjectCreateRequest.getName(),
                                appConn.getAppDesc().getAppName(),
                ...
                if (appConn instanceof OnlyStructureAppConn) {
                    ...
                    for (AppInstance appInstance : appConn.getAppDesc().getAppInstances()) {
                        ...
                        ProjectDeletionOperation operation =
                                appStandardProjectService.getProjectDeletionOperation();
                        try {
                            operation.deleteProject(requestRef);
                        } catch (ExternalOperationFailedException e) {
                            LOGGER.error(
                                    "Failed to delete project {} in {}",
                                    requestRef.getName(),
                                    appConn.getAppDesc().getAppName(),
                                    e);
                            // TODO 如果删除不了，可以先不管
    ...
    private void modifyThirdProject(
            ProjectModifyRequest projectModifyRequest, String username, String projectCreator) {
        List<DSSProjectUser> dssProjectUsers =
                projectUserService.listByPriv(
                        projectModifyRequest.getId(), ProjectUserPrivEnum.PRIV_RELEASE);
        List<String> oldReleaseUsers =
                dssProjectUsers.stream()
                        .map(DSSProjectUser::getUsername)
        ...
        // 有发布权限的用户
        projectRequestRef.setParameter(
                "releaseUsersIncreased", CollectionUtils.removeAll(releaseUsers, oldReleaseUsers));
        projectRequestRef.setParameter(
                "releaseUsersDecreased", CollectionUtils.removeAll(oldReleaseUsers, releaseUsers));
        ...
        for (AppConn appConn : AppConnManager.getAppConnManager().listAppConns()) {
            if (appConn instanceof OnlyStructureAppConn) {
                ...
                for (AppInstance appInstance : appConn.getAppDesc().getAppInstances()) {
                    ...
                    ProjectUpdateOperation operation =
                            appStandardProjectService.getProjectUpdateOperation();
                    try {
                        if (operation != null) {
                            ProjectResponseRef responseRef =
                                    operation.updateProject(projectRequestRef);
                        }
                    } catch (ExternalOperationFailedException e) {
