...
public interface DSSWorkspacePrivMapper {
    ...
    @Update(
            "update dss_menu_role set priv = #{priv}, update_time = now() "
    ...
    @Update(
            "update dss_component_role set priv = #{priv} , update_time = now()"
    ...
    @Select(
            "select count(*) from dss_component_role where workspace_id = #{workspaceId} and component_id = #{componentId} and role_id = #{roleId}")
    int queryCntOfRCP(
            @Param("workspaceId") int workspaceId,
            @Param("componentId") int componentId,
            @Param("roleId") int roleId);
