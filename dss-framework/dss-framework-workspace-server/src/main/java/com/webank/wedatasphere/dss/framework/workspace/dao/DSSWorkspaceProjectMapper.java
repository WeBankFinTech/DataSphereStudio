...
public interface DSSWorkspaceProjectMapper {
    ...
    void setProjectPriv(
            ...
            @Param("priv") int priv,
            @Param("lastUpdateTime") Date date);
    ...
    @Select(
            "select priv from dss_project_user where workspace_id = #{workspaceId} "
    ...
    */
    ...
    @Select(
            "select username from dss_flow_user where flow_id = #{flowId} and project_id = #{projectId} and workspace_id = #{workspaceId}")
    List<String> getAccessUsersByFlowId(
            @Param("flowId") Long flowId,
            @Param("projectId") Long projectId,
            @Param("workspaceId") int workspaceId);

    @Delete(
            "delete from dss_flow_user where workspace_id = #{workspaceId} and project_id = #{projectId} and flow_id = #{flowId}")
    void deleteAllWorkflowPriv(
            @Param("workspaceId") int workspaceId,
            @Param("projectId") int projectId,
            @Param("flowId") int flowId);

    @Delete(
            "delete from dss_project_user where workspace_id = #{workspaceId} and project_id = #{projectId}")
    void deleteAllProjectPriv(
            @Param("workspaceId") int workspaceId, @Param("projectId") int projectId);
    ...
    */
    ...
    @Insert(
            "insert into dss_flow_schedule_info(flow_id, schedule_time, alarm_user_emails, alarm_level) "
                    + "values(#{flowId},#{scheduleTime}, #{alarmEmails}, #{alarmLevel})")
    void setScheduleInfo(
            @Param("scheduleTime") String scheduleTime,
            @Param("alarmEmails") String alarmEmails,
    ...
    void setWorkspaceIdForProject(
            @Param("workspaceId") Integer workspaceId, @Param("projectId") Long projectId);
}
