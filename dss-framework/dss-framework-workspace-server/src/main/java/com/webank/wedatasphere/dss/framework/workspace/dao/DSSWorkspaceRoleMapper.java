...
public interface DSSWorkspaceRoleMapper {
    ...
    @Select(
            "select a.`name` from dss_role a join dss_workspace_user_role b on a.id = b.role_id"
    ...
    @Insert(
            "insert into dss_role(workspace_id, name, front_name, update_time) "
    ...
    @Select(
            "Select count(*) from dss_component_role where workspace_id = #{workspaceId} and role_id = #{roleId} and component_id = #{componentId}")
    int getCount(
    ...
    @Select(
            "select priv from dss_component_role where workspace_id = #{workspaceId} and role_id = #{roleId} and "
