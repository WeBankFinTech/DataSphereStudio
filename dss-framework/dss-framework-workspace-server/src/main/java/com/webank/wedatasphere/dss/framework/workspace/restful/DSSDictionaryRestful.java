...
public class DSSDictionaryRestful {
    ...
    @Autowired private DSSDictionaryService dictionaryService;
    ...
    public Message getDevFlowList(
            HttpServletRequest request, @RequestBody DSSDictionaryRequestVO dictionaryRequestVO) {
        try {
            if (dictionaryRequestVO.getWorkspaceId() == null) {
                return Message.error("workspaceId(空间id)不能为空");
            }
            if (StringUtils.isEmpty(dictionaryRequestVO.getParentKey())
                    && StringUtils.isEmpty(dictionaryRequestVO.getDicKey())) {
                return Message.error("获取的parentKey和dicKey不能同时为空");
            }
            List<DSSDictionary> dictionaries =
                    dictionaryService.getListByParam(dictionaryRequestVO);
            ...
            LOGGER.error(
                    "Fail to get getDevFlowListError for user {} in workspace {}",
                    dictionaryRequestVO.getWorkspaceId(),
                    e);
    ...
    public Message getDicSecondList(
            HttpServletRequest request, @RequestBody DSSDictionaryRequestVO dictionaryRequestVO) {
        try {
            if (dictionaryRequestVO.getWorkspaceId() == null) {
                return Message.error("workspaceId(空间id)不能为空");
            ...
            if (map != null && map.size() > 0 && isEnglish) {
                ...
                Map<String, List<DSSDictionary>> mapList =
                        (Map<String, List<DSSDictionary>>) map.get("mapList");
            ...
            LOGGER.error(
                    "Fail to get getDicSecondListError for user {} in workspace {}",
                    dictionaryRequestVO.getWorkspaceId(),
                    e);
