...
public class DSSWorkspaceRoleRestful {
    ...
    @Autowired DSSWorkspaceService dssWorkspaceService;
    @Autowired private DSSWorkspaceRoleService dssWorkspaceRoleService;
    @Autowired private DSSUserService userService;
    @Autowired private DSSApplicationMapper applicationMapper;
    ...
    public Message getBaseInfo(HttpServletRequest req) {
        ...
        for (DSSApplication application : applicationList) {
            ...
            if (redirectUrl != null && redirectUrl.startsWith("http")) {
                application.setHomepageUrl(
                        ApplicationUtils.redirectUrlFormat(
                                redirectUrl, application.getHomepageUrl()));
                application.setProjectUrl(
                        ApplicationUtils.redirectUrlFormat(
                                redirectUrl, application.getProjectUrl()));
                if (StringUtils.isNotEmpty(enhanceJson)
                        && enhanceJson.contains("scheduleHistory")) {
                    Gson gson = new Gson();
                    HashMap<String, Object> scheduleHistoryMap =
                            gson.fromJson(enhanceJson, HashMap.class);
    ...
    public Message getWorkspaceRoles(
            HttpServletRequest request, @RequestParam(WORKSPACE_ID_STR) int workspaceId) {
        // todo 获取工作空间中所有的角色
        List<DSSWorkspaceRoleVO> workspaceRoles =
                dssWorkspaceService.getWorkspaceRoles(workspaceId);
        return Message.ok().data("workspaceRoles", workspaceRoles);
    }

    @RequestMapping(path = "addWorkspaceRole", method = RequestMethod.POST)
    public Message addWorkspaceRole(
            HttpServletRequest request,
            @RequestBody AddWorkspaceRoleRequest addWorkspaceRoleRequest) {
        ...
        dssWorkspaceRoleService.addWorkspaceRole(
                roleName, workspaceId, menuIds, componentIds, username);
    ...
    public Message getWorkspaceInfo(
            HttpServletRequest request,
        ...
        if (roles == null || roles.isEmpty()) {
            LOGGER.error(
                    "username {}, in workspace {} roles are null or empty", username, workspaceId);
            return Message.error("can not get roles information");
        }
        // 判断如果是没有权限的，那么就直接干掉
        //        if (roles.contains("apiUser")){
        //            int priv = dssWorkspaceRoleService.getApiPriv(username, workspaceId,
        // "apiUser", "apiService");
        //            if(priv <= 0) {
        ...
        //            int priv = dssWorkspaceRoleService.getApiPriv(username, workspaceId, role,
        // "apiService");
        //            if (priv >= 1) {
        //                flag = true;
        //                break;
