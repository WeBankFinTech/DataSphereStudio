...
public class DSSWorkspaceUserRestful {
    ...
    public Message getWorkspaceUsers(
            HttpServletRequest request,
            @RequestParam(WORKSPACE_ID_STR) String workspaceId,
            @RequestParam(required = false, name = "pageNow") Integer pageNow,
        ...
        if (StringUtils.isNotEmpty(roleName)) {
            ...
            List<DSSWorkspaceUserVO> workspaceUsers =
                    dssWorkspaceService.getWorkspaceUsersByRole(
                            Integer.parseInt(workspaceId), roleName, totals, pageNow, pageSize);
            PageInfo<DSSWorkspaceUserVO> pageInfo = new PageInfo<>(workspaceUsers);
            List<DSSWorkspaceUserVO> list = pageInfo.getList();
            long total = pageInfo.getTotal();
            List<DSSWorkspaceRoleVO> dssRoles =
                    workspaceDBHelper.getRoleVOs(Integer.parseInt(workspaceId));
            return Message.ok()
                    .data("roles", dssRoles)
                    .data("workspaceUsers", list)
                    .data("total", totals.get(0));
        } else {
            List<Long> totals = new ArrayList<>();
            List<DSSWorkspaceUser01> workspaceUsers =
                    dssWorkspaceService.getWorkspaceUsers(
                            workspaceId, department, username, roleName, pageNow, pageSize, totals);
    ...
    public Message getAllWorkspaceUsers(
            HttpServletRequest request, @RequestParam(WORKSPACE_ID_STR) int workspaceId) {
        DSSWorkspaceUsersVo dssWorkspaceUsersVo = new DSSWorkspaceUsersVo();
        dssWorkspaceUsersVo.setAccessUsers(
                dssWorkspaceUserService.getAllWorkspaceUsers(workspaceId));
        dssWorkspaceUsersVo.setEditUsers(
                dssWorkspaceUserService.getWorkspaceEditUsers(workspaceId));
        dssWorkspaceUsersVo.setReleaseUsers(
                dssWorkspaceUserService.getWorkspaceReleaseUsers(workspaceId));
        return Message.ok().data("users", dssWorkspaceUsersVo);
    }

    @RequestMapping(path = "addWorkspaceUser", method = RequestMethod.POST)
    public Message addWorkspaceUser(
            HttpServletRequest request,
            @RequestBody UpdateWorkspaceUserRequest updateWorkspaceUserRequest) {
        // todo 工作空间添加用户
    ...
    public Message updateWorkspaceUser(
            HttpServletRequest request,
            @RequestBody UpdateWorkspaceUserRequest updateWorkspaceUserRequest) {
        String creator = SecurityFilter.getLoginUsername(request);
        List<Integer> roles = updateWorkspaceUserRequest.getRoles();
    ...
    public Message deleteWorkspaceUser(
            HttpServletRequest request,
            @RequestBody DeleteWorkspaceUserRequest deleteWorkspaceUserRequest) {
        // todo 删除工作空间中的用户
    ...
    public Message getWorkspaceIdByUserName(
            HttpServletRequest request,
            @RequestParam(required = false, name = "userName") String userName) {
        ...
        String userWorkspaceIdStr =
                userWorkspaceIds.stream().map(x -> x.toString()).collect(Collectors.joining(","));
