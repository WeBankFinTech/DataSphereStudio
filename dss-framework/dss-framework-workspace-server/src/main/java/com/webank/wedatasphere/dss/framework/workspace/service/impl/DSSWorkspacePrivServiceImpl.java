...
public class DSSWorkspacePrivServiceImpl implements DSSWorkspacePrivService {
    ...
    public void updateRoleMenuPriv(
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateRoleComponentPriv(
            int workspaceId, int componentId, String username, List<Pair<Integer, Boolean>> pairs) {
        pairs.forEach(
                pair -> {
                    int roleId = pair.getKey();
                    int priv = pair.getValue() ? 1 : 0;
                    int count =
                            dssWorkspacePrivMapper.queryCntOfRCP(workspaceId, componentId, roleId);
                    if (count >= 1) {
                        dssWorkspacePrivMapper.updateRoleComponentPriv(
                                workspaceId, componentId, roleId, priv);
                    } else {
                        dssWorkspacePrivMapper.insertRolComponentPriv(
                                workspaceId, componentId, roleId, priv);
                    }
                });
    }
