...
@Component
public class ExportDSSOrchestratorPluginImpl extends AbstractDSSOrchestratorPlugin
        implements ExportDSSOrchestratorPlugin {
    ...
    public Map<String, Object> exportOrchestrator(
            String userName,
        ...
        DSSOrchestratorVersion dssOrchestratorVersion =
                orchestratorMapper.getOrchestratorVersion(orcVersionId);
        DSSOrchestratorInfo dssOrchestratorInfo =
                orchestratorMapper.getOrchestrator(dssOrchestratorVersion.getOrchestratorId());
        if (dssOrchestratorInfo != null) {
            String orcExportSaveBasePath = IoUtils.generateIOPath(userName, DEFAULT_ORC_NAME, "");
            try {
                Files.createDirectories(
                        Paths.get(orcExportSaveBasePath).toAbsolutePath().normalize());
                // 标记当前导出为project导出
                ...
                LOGGER.error(
                        "Failed to export metaInfo in orchestrator server for orc({}) in version {}.",
                        orchestratorId,
                        orcVersionId,
                        e);
                DSSExceptionUtils.dealErrorException(
                        60099,
                        "Failed to export metaInfo in orchestrator server.",
                        e,
                        DSSOrchestratorErrorException.class);
            }
            LOGGER.info(
                    "{} 开始导出Orchestrator: {} 版本ID为: {} ",
                    userName,
                    dssOrchestratorInfo.getName(),
                    orcVersionId);

            // 2、导出第三方应用信息，如工作流、Visualis、Qualities
            Pair<AppInstance, DevelopmentIntegrationStandard> standardPair =
            ...
            if (null != standardPair) {
                RefExportService refExportService =
                        standardPair.getValue().getRefExportService(standardPair.getKey());

                if (refExportService == null) {
                    LOGGER.error(
                            "for {} to export orchestrator {} refExportService is null",
                            userName,
                            dssOrchestratorInfo.getName());
                    DSSExceptionUtils.dealErrorException(
                            60089, "refExportService is null", DSSErrorException.class);
                }
                OrchestratorExportRequestRef orchestratorExportRequestRef =
                        AppConnRefFactoryUtils.newAppConnRef(
                                OrchestratorExportRequestRef.class,
                                refExportService.getClass().getClassLoader(),
                                dssOrchestratorInfo.getType());
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug(
                            "orchestrator Export Request Ref class is {}",
                            orchestratorExportRequestRef.getClass());
                }
                ...
                AbstractResponseRef responseRef =
                        (AbstractResponseRef)
                                refExportService
                ...
                bmlService.downloadToLocalPath(
                        userName, resourceId, version, orcExportSaveBasePath + "orc_flow.zip");
            }
            ...
            Map<String, Object> resultMap =
                    bmlService.upload(
                            userName,
                            inputStream,
                            dssOrchestratorInfo.getName() + ".OrcExport",
                            projectName);

            // 4、判断导出后是否改变Orc的版本
            if (addOrcVersion) {
                Long orcIncreaseVersionId =
                        orchestratorVersionIncrease(
                                dssOrchestratorInfo.getId(),
                                userName,
                                dssOrchestratorInfo.getComment(),
                                workspaceName,
                                dssOrchestratorInfo,
                                projectName,
                                dssLabels);
    ...
    @Override
    public Long orchestratorVersionIncrease(
            Long orcId,
            String userName,
            String comment,
            String workspaceName,
        ...
        DSSOrchestratorVersion dssOrchestratorVersion =
                orchestratorMapper.getLatestOrchestratorVersionById(orcId);
        ...
        RefCRUDService refcrudservice =
                standardPair.getValue().getRefCRUDService(standardPair.getKey());
        if (null != refcrudservice) {
            try {
                ...
                LOGGER.error(
                        "Failed to copy new orcVersion in orchestrator server for orc({}).",
                        orcId,
                        t);
                DSSExceptionUtils.dealErrorException(
                        60099,
                        "Failed to copy app in orchestrator server",
                        t,
                        DSSOrchestratorErrorException.class);
            }
