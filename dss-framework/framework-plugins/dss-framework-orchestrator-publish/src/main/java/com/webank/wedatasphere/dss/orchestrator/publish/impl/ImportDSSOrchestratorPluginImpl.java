...
@Component
public class ImportDSSOrchestratorPluginImpl extends AbstractDSSOrchestratorPlugin
        implements ImportDSSOrchestratorPlugin {
    ...
    public Long importOrchestrator(
            String userName,
        ...
        String inputZipPath =
                IoUtils.generateIOPath(userName, projectName, DEFAULT_ORC_NAME + ".zip");
        ...
        // 2、导入Info信息(导入冲突处理)
        List<DSSOrchestratorInfo> dssOrchestratorInfos =
                metaInputService.importOrchestrator(inputPath);
        DSSOrchestratorInfo importDssOrchestratorInfo = dssOrchestratorInfos.get(0);

        // 根据工程ID和编排名称查询
        String uuid =
                orchestratorMapper.getOrcNameByParam(
                        importDssOrchestratorInfo.getProjectId(),
                        importDssOrchestratorInfo.getName());
        // 通过UUID查找是否已经导入过
        DSSOrchestratorInfo existFlag =
                orchestratorMapper.getOrchestratorByUUID(importDssOrchestratorInfo.getUUID());
        ...
        if (null != existFlag) {
            ...
            if (StringUtils.isNotBlank(uuid) && !uuid.equals(importDssOrchestratorInfo.getUUID())) {
                DSSExceptionUtils.dealErrorException(
                        61002,
                        "The same orchestration name already exists",
        ...
        Map<String, Object> resultMap =
                bmlService.upload(
                        userName,
                        inputStream,
                        importDssOrchestratorInfo.getName() + "_orc_flow.zip",
                        projectName);
        ...
        DSSOrchestrator dssOrchestrator =
                orchestratorManager.getOrCreateOrchestrator(
                        userName,
                        workspaceName,
                        importDssOrchestratorInfo.getType(),
                        importDssOrchestratorInfo.getAppConnName(),
                        dssLabels);
        ...
        Pair<AppInstance, DevelopmentIntegrationStandard> standardPair =
                OrchestratorLoaderUtils.getOrcDevelopStandard(
                        userName, workspaceName, importDssOrchestratorInfo, dssLabels);
        if (null != standardPair) {
            RefImportService refImportService =
                    standardPair.getValue().getRefImportService(standardPair.getKey());
            OrchestratorImportRequestRef orchestratorImportRequestRef = null;
            try {
                orchestratorImportRequestRef =
                        (OrchestratorImportRequestRef)
                                RefFactory.INSTANCE.newRef(
                                        OrchestratorImportRequestRef.class,
                                        orchestratorAppConn.getClass().getClassLoader(),
                                        "com.webank.wedatasphere.dss.appconn.workflow.ref");
            } catch (final Exception e) {
                DSSExceptionUtils.dealErrorException(
                        61001,
                        "failed to new ref with class "
                                + OrchestratorImportRequestRef.class.getName(),
                        e,
                        DSSErrorException.class);
            }
            ...
            CommonResponseRef responseRef =
                    (CommonResponseRef)
                            refImportService
                                    .getRefImportOperation()
                                    .importRef(orchestratorImportRequestRef);
        ...
        RequestProjectImportOrchestrator projectImportOrchestrator =
                new RequestProjectImportOrchestrator();
