...
/**
 * @author suyc @Classname GuideChapterMapper @Description TODO @Date 2022/1/13 20:31 @Created by
 *     suyc
 */
@Mapper
public interface GuideChapterMapper extends BaseMapper<GuideChapter> {
    @Select(
            "SELECT * FROM dss_guide_chapter WHERE is_delete =0 AND catalog_id =#{catalogId} ORDER BY id ASC")
    List<GuideChapter> queryGuideChapterListByCatalogId(@Param("catalogId") Long catalogId);

    @Select(
            "SELECT * FROM dss_guide_chapter WHERE is_delete =0 AND (content LIKE CONCAT('%', #{keyword}, '%') OR title LIKE CONCAT('%', #{keyword}, '%')) ORDER BY id ASC")
