...
public interface OrchestratorMapper {
    ...
    @Select(
            "select max(id) from dss_orchestrator_version_info where `orchestrator_id` = #{orchestratorId}")
    Long findLatestOrcVersionId(@Param("orchestratorId") Long orchestratorId);

    List<DSSOrchestratorVersion> getOrchestratorVersions(
            @Param("projectId") Long projectId, @Param("orchestratorId") Long orchestratorId);

    @Select(
            "select max(`version`) from dss_orchestrator_version_info where orchestrator_id = #{orchestratorId}")
    String getLatestVersion(@Param("orchestratorId") Long orchestratorId);

    @Select(
            "select uuid from dss_orchestrator_info where project_id = #{projectId} and name = #{name}")
    String getOrcNameByParam(@Param("projectId") Long projectId, @Param("name") String name);

    @Select(
            "select id from `dss_orchestrator_info` where `project_id` = #{projectId} and `is_published` = 1")
    List<Long> getAllOrcIdsByProjectId(@Param("projectId") Long projectId);

    @Select(
            "select max(`app_id`) from `dss_orchestrator_version_info` where `orchestrator_id` = #{orchestratorId} and `version` = #{version}")
    Long getAppIdByVersion(
            @Param("orchestratorId") Long orchestratorId, @Param("version") String version);
    ...
    @Delete(
            "delete from `dss_orchestrator_schedule_info` where `orchestrator_id` = #{orchestratorId}")
    int deleteScheduleInfo(@Param("orchestratorId") Integer orchestratorId);

    @Update(
            "update dss_orchestrator_schedule_info set active_flag = #{activeFlag}  where `orchestrator_id` = #{orchestratorId}")
    int updateScheduleInfoActiveFlag(
            @Param("orchestratorId") Long orchestratorId, @Param("activeFlag") String activeFlag);
    ...
    void setOrchestratorPriv(
            @Param("workspaceId") int workspaceId,
            @Param("projectId") Long projectId,
            @Param("orchestratorId") int orchestratorId,
            @Param("accessUsers") List<String> accessUsers,
            @Param("priv") int priv,
            @Param("updateTime") Date date);

    @Delete(
            "delete from `dss_orchestrator_user` "
                    + "where `workspace_id` = #{workspaceId} "
                    + "and `project_id` = #{projectId} "
                    + "and `orchestrator_id` = #{orchestratorId}")
    void deleteAllOrchestratorPriv(
            @Param("workspaceId") int workspaceId,
            @Param("projectId") Long projectId,
            @Param("orchestratorId") int orchestratorId);
