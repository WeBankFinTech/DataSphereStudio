...
public class WorkflowPersistenceEngine extends AbstractPersistenceEngine {
    ...
    @Override
    public void persist(JobRequest jobRequest)
            throws QueryFailedException, EntranceIllegalParamException, EntranceRPCException {

        if (jobRequest == null) {
            throw new EntranceIllegalParamException(
                    20004, "task can not be null, unable to do persist operation");
        }
        RequestInsertTask requestInsertTask = new RequestInsertTask();
        RequestPersistTask requestPersistTask =
                FlowExecutionUtils.jobRequest2RequestPersistTask(jobRequest);
        ...
        if (responsePersist != null) {
            ...
            // Todo can be throw exception if it is not requestPersistTask(todo 如果不是
            // requestPersistTask的话，可以进行throw异常)
        }
    }
    ...
    //    @Override
    //    public Task retrieve(Long taskID)throws EntranceIllegalParamException,
    // QueryFailedException, EntranceRPCException {
    //
    //        Task task = null;
    //
    //        if ( taskID == null ||  taskID < 0){
    //            throw new EntranceIllegalParamException(20003, "taskID can't be null or less than
    // 0");
    //        }
    //
    ...
    //        }catch(Exception e){
    //            logger.error("Requesting the corresponding task failed with taskID: {}(通过taskID:
    // {} 请求相应的task失败)", taskID, e);
    //            throw new EntranceRPCException(20020, "sender rpc failed", e);
    //        }
    //        int status = responsePersist.getStatus();
    ...
    //        String message = responsePersist.getMsg();
    //        if (status != 0){
    //            logger.error("By taskID: {} request the corresponding task return status code is
    // not 0, the query fails(通过taskID: {} 请求相应的task返回状态码不为0，查询失败)", taskID);
    ...
    //               if (list.size() == 0){
    //                   logger.info("returned list length is 0, maybe there is no task
    // corresponding to {}", taskID);
    //               }else if (list.size() == 1){
    //                   Object t = list.get(0);
    //                   Gson gson = new Gson();
    ...
    @Override
    public void updateIfNeeded(JobRequest jobRequest)
            throws EntranceRPCException, EntranceIllegalParamException {
        if (jobRequest == null) {
            throw new EntranceIllegalParamException(
        ...
        try {
            ...
            logger.error(
                    "Request to update task with taskID {} failed, possibly due to RPC failure(请求更新taskID为 {} 的任务失败，原因可能是RPC失败)",
                    requestUpdateTask.getTaskID(),
                    e);
            throw new EntranceRPCException(20020, "sender rpc failed ", e);
        }
    }
