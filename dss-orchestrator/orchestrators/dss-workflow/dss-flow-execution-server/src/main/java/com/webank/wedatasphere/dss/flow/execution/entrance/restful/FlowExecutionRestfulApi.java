    ...
    public Message execution(@PathVariable("id") String id, @RequestParam("labels") String labels) {
        ...
        try {
            if (job.isDefined() && job.get() instanceof FlowEntranceJob) {
                ...
                message.data(
                        "runningJobs",
                        FlowContext$.MODULE$.convertView(
                                flowEntranceJob.getFlowContext().getRunningNodes()));
                List<Map<String, Object>> pendingList =
                        FlowContext$.MODULE$.convertView(
                                flowEntranceJob.getFlowContext().getPendingNodes());
                pendingList.addAll(
                        FlowContext$.MODULE$.convertView(
                                flowEntranceJob.getFlowContext().getSkippedNodes()));
                message.data("pendingJobs", pendingList);
                message.data(
                        "succeedJobs",
                        FlowContext$.MODULE$.convertView(
                                flowEntranceJob.getFlowContext().getSucceedNodes()));
                message.data(
                        "failedJobs",
                        FlowContext$.MODULE$.convertView(
                                flowEntranceJob.getFlowContext().getFailedNodes()));
                message.data(
                        "skippedJobs",
                        FlowContext$.MODULE$.convertView(
                                flowEntranceJob.getFlowContext().getSkippedNodes()));
