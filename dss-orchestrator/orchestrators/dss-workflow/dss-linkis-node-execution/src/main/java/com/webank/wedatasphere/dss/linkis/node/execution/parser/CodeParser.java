...
public class CodeParser implements JobParser {
    ...
    private static final Pattern pb =
            Pattern.compile(
                    "((project)|(flow)|(node))://[^\\s\"]+[$\\s]{0,1}", Pattern.CASE_INSENSITIVE);
    ...
    public void parseJob(Job job) throws Exception {
        ...
        Map<String, Object> script =
                LinkisJobExecutionUtils.gson.fromJson(
                        linkisAppConnJob.getCode(),
        ...
        if (executionParams.get("params") != null && executionParams.get("params") instanceof Map) {
            if (linkisAppConnJob.getParams() != null) {
                linkisAppConnJob
                        .getParams()
                        .putAll((Map<String, Object>) executionParams.get("params"));
            }
        }
    }

    private Map<String, Object> getExecutionParams(
            BMLResource bmlResource, CommonLinkisJob linkisAppConnJob) {
        Map<String, Object> map = new HashMap<>();
        WorkspaceClient client =
                WorkspaceClientFactory.getClient(
                        linkisAppConnJob.getSubmitUser(),
                        LinkisJobExecutionConfiguration.LINKIS_AUTHOR_USER_TOKEN.getValue(
                                linkisAppConnJob.getJobProps()),
                        LinkisURLService.Factory.getLinkisURLService()
                                .getLinkisURL(linkisAppConnJob));
        ScriptFromBMLResponse response =
                client.requestOpenScriptFromBML(
                        bmlResource.getResourceId(),
                        bmlResource.getVersion(),
                        bmlResource.getFileName());
        linkisAppConnJob
                .getLogObj()
                .info(
                        "Get execution code from workspace client,bml resource id "
                                + bmlResource.getResourceId()
                                + ", version is "
                                + bmlResource.getVersion());
    ...
    /**
     * 1.Find the project file used in the script 2.Find the node file used in the script
     * 3.Recursively find the flow file used in the script 4.Replace file name with prefixed name
     *
     * @param resourceNames
     * @param linkisAppConnJob
     * @return
     */
    private ArrayList<BMLResource> getResourcesByNames(
            ArrayList<String> resourceNames, CommonLinkisJob linkisAppConnJob) {
        ...
        for (String resourceName : resourceNames) {
            ...
            switch (prefix) {
                case "project":
                    ...
                    afterFileName =
                            LinkisJobExecutionConfiguration.PROJECT_PREFIX
                                    + "_"
                                    + projectName
                                    + "_"
                                    + fileName;
                ...
                case "node":
                    ...
                    afterFileName =
                            LinkisJobExecutionConfiguration.JOB_PREFIX
                                    + "_"
                                    + jobName
                                    + "_"
                                    + fileName;
            ...
            if (null == resource) {
                linkisAppConnJob
                        .getLogObj()
                        .error("Failed to find the " + prefix + " resource file of " + fileName);
                throw new RuntimeException(
                        "Failed to find the " + prefix + " resource file of " + fileName);
            }
    ...
    /**
     * Recursively find the flow file used in the script Recursive exit condition is top-level flow
     */
    private BMLResource findFlowResource(
            CommonLinkisJob linkisAppConnJob, String fileName, String flowName) {

        String fullFlowName = "";
        Map<String, List<BMLResource>> fLowNameAndResources =
                linkisAppConnJob.getFlowNameAndResources();
        if (fLowNameAndResources == null) {
            return null;
        }
        Optional<Map.Entry<String, List<BMLResource>>> first =
                fLowNameAndResources.entrySet().stream()
                        .filter(
                                fLowNameAndResource ->
                                        fLowNameAndResource
                                                .getKey()
        ...
        String parentFlowName =
                StringUtils.substringAfterLast(
                        StringUtils.substringBefore(
                                fullFlowName,
                                "." + flowName + LinkisJobExecutionConfiguration.RESOURCES_NAME),
                        ".");
    ...
    private String replaceCodeResourceNames(
            String code, ArrayList<String> resourceNameList, ArrayList<BMLResource> resourceList) {
