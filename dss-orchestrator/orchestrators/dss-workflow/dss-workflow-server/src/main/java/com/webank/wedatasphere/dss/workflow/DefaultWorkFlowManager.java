...
public class DefaultWorkFlowManager implements WorkFlowManager {
    ...
    @Autowired private WorkFlowInputService workFlowInputService;

    @Autowired private WorkFlowExportService workFlowExportService;

    @Autowired private BMLService bmlService;

    @Autowired private MetaInputService metaInputService;
    @Autowired private PublishService publishService;

    private Logger LOGGER = LoggerFactory.getLogger(this.getClass());

    @Override
    public DSSFlow createWorkflow(
            String userName,
            String flowName,
            String contextIdStr,
            String description,
        ...
        String appConnJson =
                BDPJettyServerHelper.jacksonJson().writeValueAsString(linkedAppConnNames);
    ...
    @Override
    public DSSFlow copyRootflowWithSubflows(
            String userName,
            long rootFlowId,
        ...
        return flowService.copyRootFlow(
                rootFlowId, userName, workspaceName, projectName, version, contextIdStr);
    }
    ...
    public void updateWorkflow(
            String userName, Long flowId, String flowName, String description, String uses)
    ...
    @Override
    public Map<String, Object> exportWorkflow(
            String userName,
            Long flowId,
            Long dssProjectId,
            String projectName,
        ...
        DSSFlow dssFlow = flowService.getFlowByID(flowId);
        String exportPath =
                workFlowExportService.exportFlowInfo(
                        dssProjectId, projectName, flowId, userName, workspace, dssLabels);
        InputStream inputStream = bmlService.readLocalResourceFile(userName, exportPath);
        Map<String, Object> resultMap =
                bmlService.upload(
                        userName, inputStream, dssFlow.getName() + ".export", projectName);
        return resultMap;
    }

    @Override
    public List<DSSFlow> importWorkflow(
            String userName,
            String resourceId,
            String bmlVersion,
        ...
        String inputZipPath =
                IoUtils.generateIOPath(
                        userName,
                        dssFlowImportParam.getProjectName(),
                        dssFlowImportParam.getProjectName() + ".zip");
        ...
        List<DSSFlow> dwsFlowRelationList =
                workFlowInputService.persistenceFlow(
                        dssFlowImportParam.getProjectID(),
                        dssFlowImportParam.getUserName(),
                        dssFlows,
                        dwsFlowRelations,
                        dssFlowImportParam.getSourceEnv());
        List<DSSFlow> rootFlows =
                dwsFlowRelationList.stream()
                        .filter(DSSFlow::getRootFlow)
                        .collect(Collectors.toList());
        for (DSSFlow rootFlow : rootFlows) {
            workFlowInputService.inputWorkFlow(
                    dssFlowImportParam.getUserName(),
                    ...
                    inputPath,
                    null,
                    dssFlowImportParam.getWorkspace(),
                    dssFlowImportParam.getOrcVersion(),
                    dssFlowImportParam.getContextId());
        }
        return rootFlows;
    }

    @Autowired private OrchestratorMapper orchestratorMapper;

    @Override
    public ResponseOperateOrchestrator convertWorkflow(
            RequestConvertOrchestrations requestConversionWorkflow) throws DSSErrorException {
        // TODO try to optimize it by select db in batch.
        List<DSSOrchestration> flows =
                requestConversionWorkflow.getOrcAppIds().stream()
                        .map(flowService::getFlowWithJsonAndSubFlowsByID)
                        .collect(Collectors.toList());
        ...
        flows.stream()
                .forEach(
                        flow -> {
                            ...
                            releaseInfoMap.put(
                                    flow.getId(),
                                    OrchestratorReleaseInfo.newInstance(
                                            orchestratorVersion.getOrchestratorId(),
                                            orchestratorVersion.getId(),
                                            orchestratorVersion.getVersion(),
                                            flow.getId()));
                            // 如果发布过，记录调度系统中对应的工作流id
                            OrchestratorReleaseInfo releaseInfo =
                                    orchestratorMapper.getByOrchestratorId(
                                            orchestratorVersion.getOrchestratorId());
                            if (releaseInfo != null) {
                                schedulerWorkflowIdMap.put(
                                        flow.getId(), releaseInfo.getSchedulerWorkflowId());
                            }
                        });

        SchedulerAppConn appConn =
                (SchedulerAppConn)
                        AppConnManager.getAppConnManager()
                                .getAppConn(
                                        DSSWorkFlowConstant.DSS_SCHEDULER_APPCONN_NAME.getValue());
        if (appConn == null) {
            appConn = AppConnManager.getAppConnManager().getAppConn(SchedulerAppConn.class);
        }
        //        List<AppInstance> appInstances =
        // appConn.getAppDesc().getAppInstancesByLabels(requestConversionWorkflow.getDSSLabels());
        AppInstance schedulerInstance = appConn.getAppDesc().getAppInstances().get(0);
        DSSToRelConversionOperation operation =
                appConn.getOrCreateWorkflowConversionStandard()
                        .getDSSToRelConversionService(schedulerInstance)
                        .getDSSToRelConversionOperation();
        DSSToRelConversionRequestRef requestRef =
                AppConnRefFactoryUtils.newAppConnRef(
                        DSSToRelConversionRequestRef.class, appConn.getAppDesc().getAppName());
        if (requestRef instanceof ProjectToRelConversionRequestRefImpl) {
            ProjectToRelConversionRequestRefImpl relConversionRequestRef =
                    (ProjectToRelConversionRequestRefImpl) requestRef;
            relConversionRequestRef.setDSSProject(
                    (DSSProject) requestConversionWorkflow.getProject());
            relConversionRequestRef.setDSSOrcList(flows);
            relConversionRequestRef.setUserName(requestConversionWorkflow.getUserName());
            relConversionRequestRef.setWorkspace(
                    (Workspace) requestConversionWorkflow.getWorkspace());
            ...
            result.keySet().stream()
                    .forEach(
                            flowId -> {
                                ...
                                OrchestratorReleaseInfo releaseInfo =
                                        releaseInfoMap.get(workflowId);
                                OrchestratorReleaseInfo latestOrchestratorReleaseInfo =
                                        orchestratorMapper.getByOrchestratorId(
                                                releaseInfo.getOrchestratorId());

                                Long schedulerWorkflowId =
                                        Double.valueOf(String.valueOf(result.get(flowId)))
                                                .longValue();
                                if (latestOrchestratorReleaseInfo == null) { // 未发布过，插入记录
                                    ...
                                    latestOrchestratorReleaseInfo.setOrchestratorVersionId(
                                            releaseInfo.getOrchestratorVersionId());
                                    latestOrchestratorReleaseInfo.setOrchestratorVersion(
                                            releaseInfo.getOrchestratorVersion());
                                    latestOrchestratorReleaseInfo.setSchedulerWorkflowId(
                                            schedulerWorkflowId);
    ...
    @Override
    public WorkflowStatus getSchedulerWorkflowStatus(String username, Long orchestratorId)
            throws DSSErrorException {
