...
public class ContextServiceRestful {
    ...
    @Autowired private CSTableService csTableService;

    @RequestMapping(value = "tables", method = RequestMethod.POST)
    public Message tables(HttpServletRequest req, @RequestBody TablesRequest tablesRequest)
            throws DSSErrorException {
        String userName = SecurityFilter.getLoginUsername(req);
        String contextIDStr = tablesRequest.getContextID();
        String nodeName = tablesRequest.getNodeName();
        return Message.ok()
                .data("tables", csTableService.queryTables("default", contextIDStr, nodeName));
    }

    @RequestMapping(value = "columns", method = RequestMethod.POST)
    public Message queryTableMeta(
            HttpServletRequest req, @RequestBody QueryTableMetaRequest queryTableMetaRequest)
            throws DSSErrorException {
        String userName = SecurityFilter.getLoginUsername(req);
        ...
        return Message.ok()
                .data(
                        "columns",
                        csTableService.queryTableMeta("default", contextIDStr, contextKeyStr));
    }
}
