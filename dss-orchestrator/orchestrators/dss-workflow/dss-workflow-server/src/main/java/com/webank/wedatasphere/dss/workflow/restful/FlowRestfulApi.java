...
public class FlowRestfulApi {
    ...
    public Message addFlow(HttpServletRequest req, @RequestBody AddFlowRequest addFlowRequest)
        ...
        String contextId =
                contextService.createContextID(workspaceName, projectName, name, version, userName);
        DSSFlow dssFlow =
    ...
    public Message publishWorkflow(
            HttpServletRequest request,
            @RequestBody PublishWorkflowRequest publishWorkflowRequest) {
        Long workflowId = publishWorkflowRequest.getWorkflowId();
        //        Map<String, Object> labels =
        // StreamSupport.stream(Spliterators.spliteratorUnknownSize(dssLabel.getFields(),
        ...
        try {
            String taskId =
                    publishService.submitPublish(
                            publishUser, workflowId, labels, workspace, comment);
    ...
    public Message getSchedulerWorkflowStatus(
            HttpServletRequest request,
        ...
        try {
            WorkflowStatus status =
                    publishService.getSchedulerWorkflowStatus(username, orchestratorId);
            Message result =
                    Message.ok("获取调度工作流状态")
                            .data("published", status.getPublished())
                            .data("releaseStatus", status.getReleaseState());
            // Message result = Message.ok("获取调度工作流状态").data("published",
            // false).data("releaseStatus", null);
            return result;
        } catch (final Throwable t) {
            LOGGER.error(
                    "Failed to get scheduler workflow status for user {} , OrchestratorId {}",
                    username,
                    orchestratorId,
                    t);
    ...
    public Message getReleaseStatus(
            HttpServletRequest request,
            @NotNull(message = "查询的发布id不能为空")
                    @RequestParam(required = false, name = "releaseTaskId")
                    Long releaseTaskId) {
        String username = SecurityFilter.getLoginUsername(request);
        Message message;
        try {
            ResponseConvertOrchestrator response =
                    publishService.getStatus(username, releaseTaskId.toString());
            if (null != response.getResponse()) {
                ...
                if ("failed".equalsIgnoreCase(status)) {
                    message =
                            Message.error("发布失败:" + response.getResponse().getMessage())
                                    .data("status", status);
    ...
    //    @ProjectPrivChecker
    public Message updateFlowBaseInfo(
            HttpServletRequest req, UpdateFlowBaseInfoRequest updateFlowBaseInfoRequest)
            throws DSSErrorException {
    ...
    public Message get(
            HttpServletRequest req, @RequestParam(required = false, name = "flowId") Long flowID)
            throws DSSErrorException {
    ...
    //    @ProjectPrivChecker
    public Message deleteFlow(HttpServletRequest req, DeleteFlowRequest deleteFlowRequest)
            throws DSSErrorException {
        Long flowID = deleteFlowRequest.getId();
        boolean sure =
                deleteFlowRequest.getSure() != null && deleteFlowRequest.getSure().booleanValue();
        // TODO: 2019/6/13  projectVersionID的更新校验
        // state为true代表曾经发布过
    ...
    //    @ProjectPrivChecker
    public Message saveFlow(HttpServletRequest req, @RequestBody SaveFlowRequest saveFlowRequest)
        ...
        synchronized (DSSWorkFlowConstant.saveFlowLock.intern(flowID)) {
            version =
                    flowService.saveFlow(
                            flowID, jsonFlow, comment, userName, workspaceName, projectName);
        }
