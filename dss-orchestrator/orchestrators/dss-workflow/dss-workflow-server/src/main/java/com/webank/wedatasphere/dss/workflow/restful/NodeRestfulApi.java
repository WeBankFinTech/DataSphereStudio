...
public class NodeRestfulApi {
    ...
    @Autowired private FunctionInvoker functionInvoker;
    @Autowired private DSSFlowService DSSFlowService;
    ...
    public Message listNodeType(HttpServletRequest req) {
        DomainSupplier<NodeGroup, String> supplier =
                internationalization(req, NodeGroup::getNameEn, NodeGroup::getName);
    ...
    private <P, T> DomainSupplier<P, T> internationalization(
    ...
    private NodeUiValidateVO transfer(NodeUiValidate v, HttpServletRequest req) {
        DomainSupplier<NodeUiValidate, String> supplier =
                internationalization(
                        req, NodeUiValidate::getErrorMsgEn, NodeUiValidate::getErrorMsg);
    ...
    private NodeInfoVO transfer(NodeInfo nodeInfo, HttpServletRequest req)
        ...
        DomainSupplier<NodeUi, String> descriptionSupplier =
                internationalization(req, NodeUi::getDescriptionEn, NodeUi::getDescription);
        DomainSupplier<NodeUi, String> lableNameSupplier =
                internationalization(req, NodeUi::getLableNameEn, NodeUi::getLableName);
        ...
        for (NodeUi nodeUi : nodeInfo.getNodeUis()) {
            ...
            nodeUiVO.setNodeUiValidateVOS(
                    nodeUi.getNodeUiValidates().stream()
        ...
        // cache
        AppConn applicationAppConn =
                AppConnManager.getAppConnManager().getAppConn(nodeInfo.getAppConnName());
        if (applicationAppConn instanceof OnlySSOAppConn) {
            SSOIntegrationStandard standard =
                    ((OnlySSOAppConn) applicationAppConn).getOrCreateSSOStandard();
            if (null != standard) {
                SSOUrlBuilderOperation ssoUrlBuilderOperation =
                        standard.getSSOBuilderService().createSSOUrlBuilderOperation();
                ssoUrlBuilderOperation.setDSSUrl(Configuration.GATEWAY_URL().getValue());
                // todo add redirect url by labels
                ssoUrlBuilderOperation.setReqUrl(
                        applicationAppConn.getAppDesc().getAppInstances().get(0).getBaseUrl());
    ...
    public Message createExternalNode(
            HttpServletRequest req,
            @RequestBody CreateExternalNodeRequest createExternalNodeRequest)
            throws DSSErrorException, IllegalAccessException, ExternalOperationFailedException,
    ...
    public Message updateExternalNode(
            HttpServletRequest req,
            @RequestBody UpdateExternalNodeRequest updateExternalNodeRequest)
    ...
    public Message deleteExternalNode(
            HttpServletRequest req,
            @RequestBody UpdateExternalNodeRequest updateExternalNodeRequest)
            throws IllegalAccessException, ExternalOperationFailedException,
                    InstantiationException {
        ...
        if (params != null) {
            logger.info(
                    "DeletepwdExternalNode request params is " + params + ",nodeType:" + nodeType);
            node.setProjectId(projectID);
            node.setNodeType(nodeType);
            String label = updateExternalNodeRequest.getLabels().getRoute();
            params.put(DSSCommonUtils.DSS_LABELS_KEY, label);
    ...
    public Message batchDeleteAppConnNode(
            HttpServletRequest req,
            @RequestBody BatchDeleteAppConnNodeRequest batchDeleteAppConnNodeRequest)
            throws IllegalAccessException, ExternalOperationFailedException,
                    InstantiationException {
        String userName = SecurityFilter.getLoginUsername(req);
        Workspace workspace = SSOHelper.getWorkspace(req);
        ...
        jsonList.stream()
                .forEach(
                        json -> {
                            ...
                            logger.info(
                                    "DeletepwdExternalNode request params is "
                                            + params
                                            + ",nodeType:"
                                            + nodeType);
                            CommonAppConnNode node = new CommonAppConnNode();
                            ...
                            node.setNodeType(nodeType);
                            String label =
                                    ((Map<String, Object>) json.get("labels"))
                                            .get("route")
                                            .toString();
                            params.put("labels", label);
                            params.put("workspace", workspace);
                            try {
                                functionInvoker.nodeServiceFunction(
                                        userName, params, node, FunctionPool.deleteNode);
    ...
    public Message getAppConnNodeUrl(
            HttpServletRequest req, @RequestBody AppConnNodeUrlRequest appConnNodeUrlRequest)
            throws IllegalAccessException, ExternalOperationFailedException,
                    InstantiationException {
        String userName = SecurityFilter.getLoginUsername(req);
        Workspace workspace = SSOHelper.getWorkspace(req);
