...
public class PublishServiceImpl implements PublishService {
    ...
    @Override
    public String submitPublish(
            String convertUser,
            Long workflowId,
            Map<String, Object> dssLabel,
            Workspace workspace,
            String comment)
            throws Exception {
        ...
        // 2.进行提交
        try {
            RequestFrameworkConvertOrchestration requestFrameworkConvertOrchestration =
                    new RequestFrameworkConvertOrchestration();
            ...
            SchedulerAppConn schedulerAppConn =
                    (SchedulerAppConn)
                            AppConnManager.getAppConnManager()
                                    .getAppConn(
                                            DSSWorkFlowConstant.DSS_SCHEDULER_APPCONN_NAME
                                                    .getValue());
            if (schedulerAppConn == null) {
                schedulerAppConn =
                        AppConnManager.getAppConnManager().getAppConn(SchedulerAppConn.class);
            }
            // 只是为了获取是否需要发布所有Orc，这里直接拿第一个AppInstance即可。
            AppInstance appInstance = schedulerAppConn.getAppDesc().getAppInstances().get(0);
            requestFrameworkConvertOrchestration.setConvertAllOrcs(
                    schedulerAppConn
                            .getOrCreateWorkflowConversionStandard()
                            .getDSSToRelConversionService(appInstance)
                            .isConvertAllOrcs());
            ...
            DSSExceptionUtils.dealErrorException(
                    63325, "Failed to submit publish " + workflowId, t, DSSErrorException.class);
        }
        return null;
    }
    ...
    public ResponseConvertOrchestrator getStatus(String username, String taskId) {
        ...
        try {
            RequestFrameworkConvertOrchestrationStatus req =
                    new RequestFrameworkConvertOrchestrationStatus(taskId);
            response = (ResponseConvertOrchestrator) getOrchestratorSender().ask(req);
            LOGGER.info(
                    "user {} gets status of {}, status is {}，msg is {}",
                    username,
                    taskId,
                    response.getResponse().getJobStatus(),
    ...
    public WorkflowStatus getSchedulerWorkflowStatus(String username, Long orchestratorId)
        ...
        OrchestratorReleaseInfo orchestratorReleaseInfo =
                orchestratorMapper.getByOrchestratorId(orchestratorId);
        ...
        if (appInstance == null) {
            LOGGER.error(
                    "DolphinScheduler app instance is null, can not get scheduler workflow status");
            DSSExceptionUtils.dealErrorException(
                    60059,
                    "scheduler app instance is null",
                    ExternalOperationFailedException.class);
        }

        RefQueryOperation operation =
                (RefQueryOperation)
                        schedulerAppConn
                                .getOrCreateWorkflowConversionStandard()
                                .getDSSToRelConversionService(appInstance)
                                .createOperation(RefQueryOperation.class);
