...
public interface DSSJobContentRequestRef<R extends DSSJobContentRequestRef<R>>
    ...
    /**
     * When some DevelopmentOperations, such as RefCreationOperation, RefImportOperation are called, the dssJobContent will be set in, so we can use it to
     * create a one-to-one only related third appConn refJob.
     * <br>
     * This method is usually called by DSS framework, the third-part AppConn just need to use getDSSJobContent to
