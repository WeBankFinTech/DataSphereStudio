...
# 加载配置
source ${k8sDir}/config/db.sh
source ${k8sDir}/config/config.sh
...
function chooseInstallMySQLMode() {
  ...
  if [[ '2' = "$MYSQL_INSTALL_MODE" ]];then
    ENV_FLAG="dev"
    DB_CONF_PATH=${k8sDir}/tmp/dss/db
...
function executeSQL() {
  ...
  # 处理sql注释
  sed -i '/--.*/d' ${k8sDir}/tmp/dss/db/dss_ddl.sql
  sed -i '/--.*/d' ${k8sDir}/tmp/dss/db/dss_dml_real.sql
