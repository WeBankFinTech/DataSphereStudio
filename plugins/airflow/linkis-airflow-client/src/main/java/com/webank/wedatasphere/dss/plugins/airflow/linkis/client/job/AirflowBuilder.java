...
import com.webank.wedatasphere.dss.plugins.airflow.linkis.client.utils.LinkisJobTypeUtils;
 ...
 */
public class AirflowBuilder extends Builder{

    private Map<String, String> jobProps;

    private JobSignalKeyCreator jobSignalKeyCreator = new AirflowJobSignalKeyCreator();

    public AirflowBuilder setJobProps(Map<String, String> jobProps) {
    ...
    protected ReadJob createReadJob(boolean isLinkisType) {
        if(isLinkisType){
            AirflowCommonLinkisReadJob readJob = new AirflowCommonLinkisReadJob();
            readJob.setJobProps(this.jobProps);
            return readJob;
        } else {
            AirflowAppJointLinkisReadJob readJob = new AirflowAppJointLinkisReadJob();
            readJob.setJobProps(this.jobProps);
            return readJob;
        }
    }
    ...
    protected SharedJob createSharedJob(boolean isLinkisType) {
        if(isLinkisType){
            AirflowCommonLinkisSharedJob sharedJob = new AirflowCommonLinkisSharedJob();
            ...
            AirflowAppJointLinkisSharedJob sharedJob = new AirflowAppJointLinkisSharedJob();
            sharedJob.setJobProps(this.jobProps);
            int shareNum = Integer.parseInt(this.jobProps.get(LinkisJobTypeConf.SHARED_NODE_TOKEN));
            sharedJob.setSharedNum(shareNum);
            return sharedJob;
    ...
    protected SignalSharedJob createSignalSharedJob(boolean isLinkisType) {
        if(isLinkisType){
            ...
            AirflowAppJointSignalSharedJob signalSharedJob = new AirflowAppJointSignalSharedJob();
    ...
    protected LinkisJob creatLinkisJob(boolean isLinkisType) {
        if(isLinkisType){
            AirflowCommonLinkisJob linkisJob = new AirflowCommonLinkisJob();
            linkisJob.setJobProps(this.jobProps);
            return linkisJob;
        } else {
            AirflowAppJointLinkisJob linkisJob = new AirflowAppJointLinkisJob();
            linkisJob.setJobProps(this.jobProps);
            return linkisJob;
        }
    }
    ...
    private Map<String, String> getSource() {
        ...
        source.put("flowName", jobProps.get(LinkisJobTypeConf.FLOW_ID));
