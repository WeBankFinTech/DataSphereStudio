  ...
  methods: {
    ...
    run(option, cb) {
      ...
      if (option && option.id === this.script.id) {
        ...
        this.execute.on(
          'progress',
          ({ progress, progressInfo, waitingSize, yarnMetrics }) => {
    ...
    async save(auto, dataSetValue) {
      //
      this.dataSet === dataSetValue ? '' : (this.work.unsave = true)
      this.dataSet = dataSetValue
      this.work.dataSetValue = this.dataSet
      if (this.node && Object.keys(this.node).length > 0) {
        this.nodeSave()
      } else {
        const params = {
    ...
    async getLogs(option) {
      ...
      let url
      if (
        ['Succeed', 'Failed', 'Cancelled', 'Timeout'].indexOf(
          this.script.status
        ) === -1
      ) {
