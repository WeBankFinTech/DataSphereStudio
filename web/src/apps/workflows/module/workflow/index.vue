        ...
        <div class="workflow-header-search">
          ...
          <div>
            <Select
              ...
              v-model="typeFilter"
              @on-change="searchWorkflow({target:{value: searchText}}, item.id)"
            >
              <Option
                value="all"
                key="all"
  ...
  methods: {
    ...
    ProjectConfirm(orchestratorData) {
      ...
      if (this.actionType === 'add') {
        ...
        const {workspaceId, projectId, orchestratorName, orchestratorMode, orchestratorWays, id, description, uses, labels} = orchestratorData
        api.fetch(`${this.$API_PATH.PROJECT_PATH}modifyOrchestrator`, {workspaceId, projectId, orchestratorName, orchestratorMode, orchestratorWays, id, description, uses, labels}, 'post').then(() => {
          this.loading = false;
    ...
    searchWorkflow(event, id) {
      ...
      const filterOwner = this.typeFilter === 'owner'
      this.dataList = tepArray.map((item) => {
        if (id === item.id) {
          item.dwsFlowList = item.dwsFlowList.filter((subItem) => {
            let flag = event.target.value ? subItem.orchestratorName.indexOf(event.target.value) != -1 : true;
            if (filterOwner) {
              flag = flag && subItem.createUser === this.getUserName();
            }
            return flag
          });
        }
