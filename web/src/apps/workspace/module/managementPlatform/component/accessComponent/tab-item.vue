      ...
      <Form :model="formItem"  label-position="top" :rules="ruleValidate" ref="formValidate">
        <FormItem prop="onestop_menu_id">
          ...
          <Select v-model="formItem.onestop_menu_id">
            <Option value="dataAccess">数据接入</Option>
            <Option value="dataanAlysis">应用开发</Option>
            <Option value="productionOperation">数据服务</Option>
            <Option value="dataQuality">数据分析</Option>
            <Option value="administratorFunction">生产运维</Option>
          </Select>
        </FormItem>
        ...
        <FormItem prop="desc_cn">
          ...
          <Input v-model="formItem.desc_cn" type="textarea" :autosize="{minRows: 6,maxRows: 10}" placeholder="请输入描述"></Input>
        </FormItem>
        <FormItem prop="desc_en">
          <span class="form-item-label">英文描述</span>
          <Input v-model="formItem.desc_en" type="textarea" :autosize="{minRows: 6,maxRows: 18}" placeholder="请输入英文描述"></Input>
        </FormItem>
        <!-- <FormItem>
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        title_en: [
          { required: true, message: '组件/应用名英文不能为空', trigger: 'blur' }
        ],
        url: [
          { required: true, message: 'baseurl不能为空', trigger: 'blur' }
        ],
        homepage_url: [
