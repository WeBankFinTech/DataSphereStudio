  ...
  <Poptip
    ...
    <div slot="content">
      ...
      <Form
        ...
        :label-width="100"
        size="small">
        <FormItem :label="$t('message.functionView.formItems.name') + '：'">
          <span
            :title="node.name"
            class="fn-item">{{ node.name }}</span>
        </FormItem>
        <FormItem :label="$t('message.functionView.formItems.createUser') + '：'">
          <span>{{ node.createUser }}</span>
        </FormItem>
        <FormItem :label="$t('message.functionView.formItems.updateTime') + '：'">
          <span>{{ dateFormatter(node.updateTime) }}</span>
        </FormItem>
        <!--<FormItem label="注册格式：">
                    <span>{{node.registerFormat}}</span>
                </FormItem>-->
        <FormItem :label="$t('message.functionView.formItems.useFormat') + '：'">
          <span
            :title="node.useFormat"
            class="fn-item">{{ node.useFormat }}</span>
        </FormItem>
        <FormItem :label="$t('message.functionView.formItems.description') + '：'">
          <span
