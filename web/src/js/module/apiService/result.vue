  ...
  <div
    ...
    <div
      ...
      <div v-if="result.bodyRows">
        <div
          v-for="(row, index) in result.bodyRows"
          :key="index">{{ row[0] }}</div>
      </div>
      <span
        v-else
...
import WeToolbar from '@/js/module/workbench/script/toolbar.vue';
import elementResizeEvent from '@/js/helper/elementResizeEvent';
import resultSetList from '@/js/module/workbench/script/resultSetList.vue';
export default {
  components: {
