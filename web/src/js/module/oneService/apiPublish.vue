...
export default {
  ...
  data() {
    ...
    return {
      ...
      paramInfoColumns: [
        ...
        {
          title: this.$t('message.oneService.apiPublish.paramTable.displayName'),
          key: 'displayName',
          width: '200',
          render: (h, params) => {
            return h('div', [
  ...
  methods: {
    ...
    saveApiOk() {
      if(!this.isOk){
        this.$Message.error(this.$t("message.oneService.apiPublish.paramTable.displayName"))
        return
      }
      this.$emit('on-save');
